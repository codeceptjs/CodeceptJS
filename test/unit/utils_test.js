...
describe('utils', () => {
  ...
  describe('#screenshotOutputFolder', () => {
    ...
    it('returns the given filename for absolute one', () => {
      const absolutePath = path.resolve('/Users/someuser/workbase/project1/test_output/screenshot1.failed.png')
      const _path = utils.screenshotOutputFolder(absolutePath)
      expect(_path).eql(absolutePath)
    })

    it('returns path inside output dir for relative path with subdirectory', () => {
      const _path = utils.screenshotOutputFolder('custom/debug.png')
      expect(_path).eql(path.join('/Users/someuser/workbase/project1/test_output', 'custom', 'debug.png'))
    })
  })
