...
Data(accounts1).Scenario('Should log accounts1', ({ current }) => {
  console.log(`Got login ${current.login} and password ${current.password}`);
});

Data(accounts2).Scenario('Should log accounts2', ({ current }) => {
...
Data(function* () {
  yield ['nick', 'pick'];
  yield ['jack', 'sacj'];
}).Scenario('Should log accounts3', ({ current }) => {
...
Data(function* () {
  yield { user: 'nick' };
  yield { user: 'pick' };
}).Scenario('Should log accounts4', ({ current }) => {
  console.log(`Got generator login ${current.user}`);
});

Data(['1', '2', '3']).Scenario('Should log array of strings', ({ current }) => {
