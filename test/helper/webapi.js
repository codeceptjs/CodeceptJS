...
`,
  ...
  describe('#saveScreenshot', () => {
    ...
    it('should create a screenshot file in custom dir', async () => {
      ...
      const sec = new Date().getUTCMilliseconds()
      global.custom_dir = path.join(global.output_dir, 'custom')
      await I.amOnPage('/')
    ...
    it('should create a screenshot in subdirectory of output dir', async () => {
      if (isHelper('TestCafe')) return

      const sec = new Date().getUTCMilliseconds()
      await I.amOnPage('/')
      await I.saveScreenshot(`subfolder/screenshot_${sec}.png`)
      assert.ok(fileExists(path.join(global.output_dir, 'subfolder', `screenshot_${sec}.png`)), null, 'file does not exists')
    })
  })
