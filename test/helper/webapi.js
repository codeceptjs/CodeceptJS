...
`,
    ...
    describe('#saveScreenshot', () => {
        ...
        it('should create a full page screenshot file in output dir', async () => {
            ...
            assert.ok(
                fileExists(path.join(global.output_dir, `screenshot_full_${+sec}.png`)),
                null,
                'file does not exists',
            )
        })
    ...
    describe('#seeAttributesOnElements', () => {
        ...
        it('should verify the boolean attribute', async function () {
            ...
            try {
                ...
                e.message.should.include(
                    'expected all elements (input) to have attributes {"disabled":true} "0" to equal "1"',
                )
            }
        })
    })
    ...
    describe('#seeCssPropertiesOnElements', () => {
        ...
        it('should check css property for several elements', async function () {
            if (isHelper('TestCafe') || process.env.BROWSER === 'firefox' || process.env.DevTools === 'true')
                this.skip()
