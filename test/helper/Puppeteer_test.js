...
describe('Puppeteer', function () {
    ...
    describe('#waitForFunction', () => {
        ...
        it('should pass arguments and wait for function returns true', () => {
            return I.amOnPage('/form/wait_js').then(() =>
                I.waitForFunction((varName) => window[varName], ['__waitJs'], 3),
            )
        })
    })
        ...
        it('should wait for a specified number of elements on the page', async () => {
            try {
                ...
                e.message.should.include(
                    'The number of elements (//div[@id = "grab-multiple"]//a) is not 2 after 0.1 sec',
                )
            }
        })
    ...
    describe('#seeInSource, #grabSource', () => {
        ...
        it('should grab the source', () =>
            I.amOnPage('/')
                ...
                .then((source) =>
                    assert.notEqual(
                        source.indexOf('<title>TestEd Beta 2.0</title>'),
                        -1,
                        'Source html should be retrieved',
                    ),
                ))
    })
            ...
            I.amOnPage('/')
                ...
                .catch((e) => {
                    ...
                    e.message.should.be.equal(
                        'expected element h1 "Welcome to test app" to equal "Welcome to test app!"',
                    )
                }))
    })
    ...
    describe('#grabHTMLFrom', () => {
        ...
        it('should grab inner html from an element using id query', () =>
            I.amOnPage('/')
                ...
                .then((html) =>
                    assert.equal(html.trim(), '<a href="/form/file" qa-id="test" qa-link="test"> Test Link </a>'),
                ))
    ...
    describe('#waitForClickable', () => {
        ...
        it('should fail for disabled element by XPath', async () => {
            ...
            await I.waitForClickable({ xpath: './/button[@id="button"]' }, 0.1)
                ...
                .catch((e) => {
                    e.message.should.include(
                        'element {xpath: .//button[@id="button"]} still not clickable after 0.1 sec',
                    )
                })
        })
        ...
        xit('should fail if element change class and not clickable', async () => {
            ...
            await I.waitForClickable('//button[@name="button_publish"]', 0.1)
                ...
                .catch((e) => {
                    e.message.should.include(
                        'element //button[@name="button_publish"] still not clickable after 0.1 sec',
                    )
                })
        })
    })
