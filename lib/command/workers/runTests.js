...
function collectStats() {
  ...
  event.dispatcher.on(event.test.passed, (testOrSuite) => {
    ...
    const testIndex = failedTests.findIndex(failed => id === failed);
    if (testIndex !== -1) {
      failedTests.splice(testIndex, 1);
      if (stats.failures > 0) {
        stats.failures--;
      }
    }
  ...
  event.dispatcher.on(event.test.failed, (testOrSuite) => {
    const id = testOrSuite.id || (testOrSuite.ctx && testOrSuite.ctx.test.id) || 'empty';
    if (!failedTests.includes(id)) {
      failedTests.push(id);
      stats.failures++;
    }
  });
  event.dispatcher.on(event.test.finished, (testOrSuite) => {
    const id = testOrSuite.id || (testOrSuite.ctx && testOrSuite.ctx.test.id) || 'empty';
    const test = runTests.find(runTest => id === runTest);
    if (!test) {
      runTests.push(id);
      stats.tests++;
    }
