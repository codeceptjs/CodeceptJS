...
// codeceptjs run:multiple 'chrome: smoke, firefox: regression' - will launch smoke suite in chrome and regression in firefox
// codeceptjs run:multiple 'chrome: smoke, regression' - will launch smoke suite in chrome and regression in firefox and chrome
...
module.exports = function (suites, options) {
  ...
  config = getConfig(testRoot, configFile);
  ...
  suites.forEach(function (suite) {
    ...
    runSuite(suite, suiteConf, browser);
  });
};
...
function runSuite(suite, suiteConf, browser) {
  ...
  browserConfig = suiteConf.browsers
    .map((b) => typeof b == 'object' ? b : { browser: b })
    .filter((b) => b.browser == browser)[0];

  for (let key in Object.keys(browserConfig)) {
