...
module.exports = function (preferredRuns, options) {
  ...
  preferredRuns = options.all ? Object.keys(config.multiple) : preferredRuns;
  if (!preferredRuns.length) {
  ...
  const runsToExecute = [];
  collection.createRuns(preferredRuns, config).forEach((run) => {
