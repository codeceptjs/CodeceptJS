  ...
  return inquirer.prompt([
    {
      ...
      type: 'confirm',
      default: false,
      message: `Do you plan to write ${kind} object in TypeScript?`,
    },
    {
      type: 'input',
      name: 'name',
    ...
    },
    {
      ...
      default: answers => `./${kind}s/${answers.name}.${answers.typescript ? 'ts' : 'js'}`,
    }]).then((result) => {
    extension = result.typescript ? 'ts' : 'js';
    ...
    let configFile = path.join(testsPath, 'codecept.conf.js');

    if (!fileExists(configFile)) {
      configFile = path.join(testsPath, 'codecept.conf.ts');
    }
