...
module.exports.pageObject = function (genPath, opts) {
        ...
        .then((result) => {
            ...
            if (config.include.I) {
                ...
                if (actorPath.charAt(0) === '.') {
                    // relative path
                    actorPath =
                        path.relative(dir, path.dirname(path.join(testsPath, actorPath))) + actorPath.substring(1) // get an upper level
                }
    ...
    ...
      ${name}: '${result.filename}',
            ...
            output.print(
                `Use ${output.colors.bold(colors.cyan(name))} as parameter in test scenarios to access this object:`,
            )
