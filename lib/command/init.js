...
let extension = 'js';
let installGitHubActions = false;
let helperName;
...
module.exports = function (initPath) {
  ...
  inquirer.prompt([
    ...
    },
    {
      ...
      type: 'confirm',
      default: false,
      message: 'Would you prefer to explicitly use `await` for each I.* commands?',
    ...
    installGitHubActions = result.installGitHubActions;
    ...
    const finish = async () => {
      ...
      if (installGitHubActions === true) {
        const ghaYml = `name: Run Tests with ${helperName}
...
`;
        const ghaPath = path.join(testsPath, '.github/workflows');
        mkdirp.sync(ghaPath);
        const ghaFile = path.join(ghaPath, 'run-tests.yml');
        fs.writeFileSync(ghaFile, ghaYml);
        print(`Github Actions workflow is created at ${ghaPath}`);
      }
