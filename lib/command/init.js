...
let configHeader = `const { setHeadlessWhen, setCommonPlugins } = require('@codeceptjs/configure');
...
module.exports = function (initPath) {
  ...
  inquirer.prompt([
    ...
    if (result.typescript === 'Yes') {
      isTypeScript = true;
      packages.push('typescript');
      packages.push('ts-node');
      configHeader = tsNodeRequired + configHeader;
    }
    ...
    const finish = async () => {
      ...
      const stepFile = isTypeScript ? './steps_file.ts' : './steps_file.js';
      ...
      let tsconfigFile = '';
      if (isTypeScript) {
        tsconfigJson = beautify(JSON.stringify(tsconfig));
        tsconfigFile = path.join(testsPath, 'tsconfig.json');
      }
