...
const grepFile = (file, grep) => {
  ...
  return !!pattern.exec(contents.toString('utf8'));
};
 ...
 * the splitting.
 */
const createChunks = ({ chunks, ...config }, patterns = []) => {
  ...
  if (typeof chunks === 'function') {
    createdChunks = chunks.call(this, files);
  } else if (typeof config.chunks === 'number' || typeof chunks === 'string') {
    createdChunks = splitFiles(files, Math.ceil(files.length / chunks));
  } else {
    throw new Error('chunks is neither a finite number or a valid function');
  }

  return createdChunks.map((chunkFiles) => {
    const { js, gherkin } = mapFileFormats(chunkFiles);
    return { ...config, tests: flattenFiles(js), gherkin: { features: flattenFiles(gherkin) } };
  });
};
