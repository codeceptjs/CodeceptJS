...
module.exports = function (genPath, options) {
  ...
  codecept.init(testsPath, (err) => {
    ...
    for (const name in helpers) {
      ...
      addAllMethodsInObject(helper, actions, methods, translations);
    }
    addAllNamesInObject(suppportI, actions, methods);

    const supports = container.support(); // return all support objects
    let exportPageObjects = [];
    let callbackParams = [];
    for (const name in supports) {
      ...
      let pageMethods = [];
      addAllMethodsInObject(pageObject, {}, pageMethods);
