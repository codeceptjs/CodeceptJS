...
module.exports.getConfig = async function (configFile) {
  try {
    return await require('../config').load(configFile)
  } catch (err) {
    fail(err.stack)
  }
}

module.exports.getConfigSync = function (configFile) {
  try {
    return require('../config').loadSync(configFile)
  } catch (err) {
    fail(err.stack)
  }
}
