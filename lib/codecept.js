...
class Codecept {
  ...
  init(dir, callback) {
    ...
    if (!this.config.noGlobals) {
      ...
      // BDD
      const stepDefinitions = require('./interfaces/context');
  ...
  loadTests(pattern) {
    ...
    if (!pattern) {
      patterns = [];
      if (this.config.tests) patterns.push(this.config.tests);
      if (this.config.gherkin.features) patterns.push(this.config.gherkin.features);
    }
