...
const defaultPrompts = {
  ...
  healStep: (html, { step, error, prevSteps }) => [
    ...
    { role: 'user', content: `I want to heal a test that fails. Here is the list of executed steps: ${prevSteps.map(s => s.toString()).join(', ')}` },
    ...
    { role: 'user', content: `Here is the error message: ${error.message}` },
...
class AiAssistant {
  ...
  connectToEvents() {
    event.dispatcher.on(event.all.result, () => {
      if (this.isEnabled && this.numTokens > 0) {
        ...
        output.print(`AI assistant used approx ~${numTokensK}K input tokens in this session. Tokens limit: ${maxTokensK}K`);
      }
    });
  }
  ...
  calculateTokens(messages) {
    // we implement naive approach for calculating tokens
