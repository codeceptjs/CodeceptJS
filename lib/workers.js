...
const createWorker = (workerObject, isPoolMode = false) => {
  const worker = new Worker(pathToWorker, {
    workerData: {
      ...
      poolMode: isPoolMode,
      failedTestsData: workerObject.failedTestsData,
    },
  })
...
const createWorkerObjects = (testGroups, config, testRoot, options, selectedRuns, failedTestsData) => {
  ...
  if (selectedRuns === undefined || !selectedRuns.length || config.multiple === undefined) {
    return testGroups.map((tests, index) => {
      ...
      if (failedTestsData) {
        workerObj.addFailedTestsData(failedTestsData)
      }
      return workerObj
    })
  }
...
class WorkerObject {
  ...
  addFailedTestsData(failedTestsData) {
    this.failedTestsData = failedTestsData
  }
}
...
class Workers extends EventEmitter {
  ...
  constructor(numberOfWorkers, config = { by: 'test' }) {
    ...
    // Handle failed tests filtering
    if (config.failedTestsData) {
      this.codecept.filterByFailedTests(config.failedTestsData.tests)
    }
  ...
  _initWorkers(numberOfWorkers, config) {
    ...
    this.workers = createWorkerObjects(this.testGroups, this.codecept.config, config.testConfig, config.options, config.selectedRuns, config.failedTestsData)
