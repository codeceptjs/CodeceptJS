...
class Workers extends EventEmitter {
  ...
  createGroupsOfTests(numberOfWorkers) {
    ...
    mocha.suite.eachTest((test) => {
      if (test) {
        mocha.suite.addTest(test.clone());
      }
    });
    mocha.suite.eachTest((test) => {
      if (test) {
  ...
  _updateFinishedTests(test) {
    ...
    if (this.finishedTests[id]) {
      const maxReruns = this.codecept.config.rerun && this.codecept.config.rerun.maxReruns;

      if (this.finishedTests[id].runs <= maxReruns) {
        this.finishedTests[id].runs++;
      }
      if (test.retries !== -1) {
