...
module.exports = function(config) {
  ...
  event.dispatcher.on(event.step.after, (step) => {
    recorder.add(`screenshot of failed test`, async () => await persistStep(step), true);
  });
  ...
  function persist(test, err) {
    ...
    for (let i in slides) {
      let step = slides[i];
      let stepNum = parseInt(i, 10);
      indicatorHtml += template(indicatorTemplate, {
        step: stepNum,
        isActive: stepNum ? '' : 'class="active"'
      });

      slideHtml += template(slidesTemplate, {
        image: i,
    ...
    const html = template(globalTemplate, {
  ...
  function isStepIgnored(step) {
    ...
    for (let pattern of config.ignoreSteps || []) {
