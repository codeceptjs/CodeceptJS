...
module.exports = function (config) {
  ...
  event.dispatcher.on(event.step.started, (step) => {
    ...
    step.id = uuidv4();
    console.log(step);
    const t = testStartedAt;

    steps[step.id] = {
      start: formatTimestamp(c - t),
      startDate: c,
      counter: stepCounter,
      title: `${step.actor}.${step.name}(${step.args.join(',')})`,
    };
    stepCounter += 1;
  });
  ...
  event.dispatcher.on(event.test.after, async (test) => {
    if (test.artifacts.video) {
      ...
      sorted.sort((stepA, stepB) => {
        return stepA.startDate - stepB.startDate;
      });
...
`;
      ...
      const onlyPath = require('path').dirname(test.artifacts.video);
      const onlyFile = require('path').basename(test.artifacts.video, '.webm');

      await fsPromises.writeFile(`${onlyPath}/${onlyFile}.srt`, str);
      test.artifacts.subtitle = `${onlyPath}/${onlyFile}.srt`;
    }
  });
};
