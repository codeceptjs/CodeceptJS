...
let testStartedAt;
/**
 * Automatically captures steps as subtitle, and saves it as an artifact when
 * video is found for a failed test
 *
 ...
 * ```
 *
 * `captureStepAsSubtitle` plugin should only be enabled when the `video:true`
 * is enabled for playwright
 */
...
`;
      ...
      await fsPromises.writeFile(`${artifactsDirectory}/${fileName}.srt`, subtitle);
