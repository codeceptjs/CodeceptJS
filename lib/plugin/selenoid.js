...
const supportedHelpers = ['WebDriver', 'Puppeteer'];
...
const selenoid = (config) => {
  ...
  event.dispatcher.on(event.all.before, () => {
    switch (helperName) {
      ...
      case 'Puppeteer': setOptionsForPuppeteer(config); break;
    }
  });
...
function setOptionsForPuppeteer(config) {
  const { remote } = require('webdriverio');
  const Puppeteer = container.helpers('Puppeteer');
  let browser = null;

  const lauchPuppeteer = (test) => {
    console.log('before lauch');
      ...
      browser = await remote({
        ...
        capabilities: {
          ...
          'selenoid:options': {
            ...config,
            name: test.title,
            videoName: `${test.title}.mp4`,
            logName: `${test.title}.log`,
          },
        },
      });
      console.log(browser.sessionId);
      Puppeteer.isRemoteBrowser = true;
      Puppeteer.puppeteerOptions = { browserWSEndpoint: `ws://localhost:4444/devtools/${browser.sessionId}/` };
    });
  };

  // Launch browser for every test before

  event.dispatcher.on(event.test.before, lauchPuppeteer);

  event.dispatcher.on(event.test.after, () => {
    recorder.add('Closinng browser', () => Puppeteer._stopBrowser());
