...
const { initialiseServices } = require('@wdio/utils');
 ...
 * ###### Setup
 *
 * 1. Install `@wdio/utils` package compatible with your webdriverio version (should be the same major+minor number).
 * 2. Install a webdriverio service
 * 3. Enable `wdio` plugin in config
 * 4. Add service name to `services` array inside wdio plugin config.
 *
 * See examples below:
 *
...
module.exports = (config) => {
  ...
  const launchers = initialiseServices(config, config.capabilities || {}, 'launcher');
  const services = initialiseServices(config, config.capabilities || {});
  ...
  function registerService(service) {
    ...
    if (service.beforeTest) {
      event.dispatcher.on(event.test.started, async (test) => {
        ...
        if (webDriver) {
          ...
          global.browser.config = Object.assign(mainConfig.get(), global.browser.config);
        }
    ...
    if (restartsSession && service.before) {
      event.dispatcher.on(event.test.started, test => service.before());
    }

    if (restartsSession && service.after) {
      event.dispatcher.on(event.test.finished, test => service.after());
    }
