  ...
  function generateTestsHtml(tests, config) {
    ...
    return tests
      .map(test => {
        ...
        return `
        <div class="test-item ${statusClass}" id="test-${test.id}" data-feature="${escapeHtml(feature)}" data-status="${statusClass}" data-tags="${(test.tags || []).join(',')}" data-retries="${test.retryAttempts || 0}">
