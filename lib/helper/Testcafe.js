...
const { mapError, createTestFile } = require('./testcafe/testcafe-utils');
...
class Testcafe extends Helper {
  ...
  async _startBrowser() {
    this.dummyTestcafeFile = createTestFile(); // create a dummy test file to get hold of the test controller
  ...
  async see(text, context = null) {
    ...
    if (context) {
      ...
      // TODO Check how that is implemented in Puppeteer
      els = findElements.call(this, this.context, `//*[contains(text(),'${text}')]`);
    }

    return this.t
      .expect(els.count).gt(0, `No element with text "${text}" found`)
      .catch(mapError);
  }
  ...
  async waitForFunction(fn, argsOrSec = null, sec = null) {
    ...
    const browserFn = ClientFunction(fn).with({ boundTestRun: this.t });
