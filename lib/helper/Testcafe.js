...
function findElements(matcher, locator) {
  console.log('findElements', matcher, locator);
  ...
  return matcher ?
    matcher.find(elementByXPath(locator.value).with({ boundTestRun: this.t })) :
    elementByXPath(locator.value).with({ boundTestRun: this.t });
}
...
async function findCheckable(locator, context) {
  ...
  if (typeof context === 'string') {
    ...
    console.log('findCheckable: Found context el', contextEl);
  }

  const matchedLocator = new Locator(locator);
  if (!matchedLocator.isFuzzy()) {
    console.log('findCheckable: !fuzzy');
  ...
  console.log('findCheckable: byText');
  if (await els.count) {
    return els;
  }
  ...
  if (await els.count) {
    console.log('findCheckable: byName return', await els.count);
    return els;
  }

  console.log('findCheckable: default');
