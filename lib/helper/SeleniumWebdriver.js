...
class SeleniumWebdriver extends Helper {
  constructor(config) {
    console.log('SeleniumWebdriver helper is DEPRECATED. Switch to Protractor instead');
    super(config);
  ...
  async executeAsyncScript(fn) {
    await this.browser.manage().timeouts().setScriptTimeout(this.options.scriptTimeout);
  ...
  async saveScreenshot(fileName, fullPage = false) {
    ...
    if (!fullPage) {
      let png = this.browser.takeScreenshot();
...
function guessLocator(locator) {
  ...
  if (this.type) return global.by[this.type](this.value);
  return false;
}
