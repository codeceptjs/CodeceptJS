...
class FileSystem extends Helper {
  ...
  /**
  * Checks that any file exists including given text
  */
  seeFileNameMatching(text) {
    assert.ok(this.getFileNames().some(file => file.includes(text)),
      `File contains ${text} not found in ${this.dir}`);
  }
  ...
  /**
  * Returns filenames without directories. 
  */
  getFileNames() {
