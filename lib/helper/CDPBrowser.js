...
class CDPBrowser extends Helper {
  ...
  _ensureLifecycleListener() {
    ...
    this.cdp.on('Page.lifecycleEvent', (params, sessionId) => {
      ...
      if (params.name !== 'load') return
      this._pageLoadWaiters = this._pageLoadWaiters.filter(waiter => {
        if (waiter.loaderId !== params.loaderId) return true
        waiter.resolve()
        return false
      })
    })
   ...
   * @protected
   */
  _waitForLoadEvent(loaderId, timeoutSec) {
   ...
   *
   * @param {?string} loaderId - loader id from the triggering `Page.navigate` response, if any.
  ...
  async _waitForPageLoad(loaderId, timeoutMessage) {
    const readyStateCheck = () => this._evaluate(`document.readyState === 'complete'`).catch(() => false)
    if (!loaderId) return this._poll(readyStateCheck, this.options.getPageTimeout, timeoutMessage)

    const cancelToken = { cancelled: false }
    const pollPromise = this._poll(readyStateCheck, this.options.getPageTimeout, timeoutMessage, cancelToken)
    const { promise: lifecyclePromise, cancel: cancelLifecycle } = this._waitForLoadEvent(loaderId, this.options.getPageTimeout)
    try {
   ...
   * @param {?object} selection - `{index}` or `{strict: true}`, mirroring `_selectionDescriptor`.
