...
class CDPBrowser extends Helper {
  ...
  async _runSelected(candidates, action, payload, selection, context = null) {
    ...
    const res = await this._evaluate(
      `window.__codecept.run(${JSON.stringify(candidates)}, ${JSON.stringify(action)}, ${JSON.stringify(payload || null)}, ${JSON.stringify(scope)}, ${JSON.stringify(selection)})`,
    )
    if (res?.outOfBounds) {
   ...
   *
   * @param {CodeceptJS.LocatorOrString} locator element located by CSS|XPath|strict locator.
   * @returns {Promise<WebElement>} a WebElement instance.
   * @throws {ElementNotFound} if no element matches `locator`.
   */
