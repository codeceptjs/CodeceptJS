...
class CDPBrowser extends Helper {
   ...
   * exercises the same input pipeline a real user would. Otherwise it delegates to `forceClick`.
   *
   * ```js
   * // simple link
   * I.click('Logout');
  ...
  async waitForElement(locator, sec = null) {
    ...
    return this._poll(
      async () => (await this._run(candidates, 'count')).found > 0,
      timeout,
  ...
  async waitInUrl(urlPart, sec = null) {
    ...
    try {
      return await this._poll(
        async () => {
          lastUrl = await this.grabCurrentUrl()
