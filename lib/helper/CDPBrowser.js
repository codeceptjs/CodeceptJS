...
class CDPBrowser extends Helper {
  ...
  async _seeInField(assertType, field, value) {
    ...
    if (typeof value === 'boolean') {
      return equals(`fields by ${locatorText}`)[assertType](value, values.length > 0)
    }
  ...
  async grabCssPropertyFromAll(locator, cssProperty) {
    const res = await this._run(this._candidates(locator), 'cssProps')
    if (!res.found) return []
  ...
  async waitForCookie(name, sec = null) {
    const timeout = sec || this.options.waitForTimeout
    try {
      return await this._poll(async () => (await this.grabCookies()).some(c => c.name === name), timeout, 'placeholder')
    } catch (e) {
      throw new Error(`Cookie ${name} is not found after ${timeout}s`)
    }
  }
