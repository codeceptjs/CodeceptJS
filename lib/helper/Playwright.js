...
class Playwright extends Helper {
  ...
  async switchTo(locator) {
    ...
    try {
      if (this.frame) {
        this.frame = await Promise.race([this.frame.frameLocator(locator), new Promise((_, reject) => setTimeout(() => reject(new Error('Child frame locator timeout')), 5000))])
      } else {
        this.frame = await Promise.race([this.page.frameLocator(locator), new Promise((_, reject) => setTimeout(() => reject(new Error('Page frame locator timeout')), 5000))])
      }
    } catch (e) {
