 ...
 * @prop {{ recordMode: 'minimal', content: 'embed' }} [har] - HAR recording. See more in HAR Recording Customization section.
 */
const config = {};
...
class Playwright extends Helper {
  ...
  async _stopBrowser() {
    ...
    this.frame = null;
    popupStore.clear();
    if (this.options.har) await this.browserContext.close();
  ...
  async amOnPage(url) {
    ...
    if (this.options.har) {
      const harExt = this.options.har.content && this.options.har.content === 'attach' ? 'zip' : 'har';
      ...
      await this.page.routeFromHAR(fileName, {
        update: true,
        updateMode: this.options.har.recordMode || 'minimal',
        updateContent: this.options.har.content || 'embed',
      });
