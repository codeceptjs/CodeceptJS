...
class Playwright extends Helper {
    ...
    try {
      ...
      for (const [strategyName, strategyFunction] of globalCustomLocatorStrategies.entries()) {
        if (!registeredCustomLocatorStrategies.has(strategyName)) {
          try {
            this.debug(`Registering custom locator strategy: ${strategyName}`)
            ...
            const createCustomEngine = ((name, func) => {
              return () => {
                ...
                return {
                  ...
                  query(root, selector) {
                    this.debug(`Query called for ${name} with selector: ${selector}, root:`, root?.tagName)
                    try {
                      if (!root) return null
                      const result = func(selector, root)
                      this.debug(`Query result for ${name}:`, result?.tagName || result)
                  ...
                  queryAll(root, selector) {
                    this.debug(`QueryAll called for ${name} with selector: ${selector}, root:`, root?.tagName)
                    try {
                      if (!root) return []
                      const result = func(selector, root)
                      this.debug(`QueryAll result for ${name}:`, result?.length || result?.tagName || result)
            ...
            this.debug(`Successfully registered custom locator strategy: ${strategyName}`)
          } catch (error) {
            if (!error.message.includes('already registered')) {
              console.warn(`Failed to register custom locator strategy '${strategyName}':`, error)
            } else {
              this.debug(`Custom locator strategy '${strategyName}' already registered`)
            }
          }
        }
      }
  ...
  async _registerCustomLocatorStrategies() {
    ...
    for (const [strategyName, strategyFunction] of Object.entries(this.customLocatorStrategies)) {
      if (!registeredCustomLocatorStrategies.has(strategyName)) {
        try {
          this.debug(`Registering custom locator strategy for context: ${strategyName}`)
          ...
          await playwright.selectors.register(strategyName, createCustomEngine)
          registeredCustomLocatorStrategies.add(strategyName)
          this.debug(`Successfully registered custom locator strategy for context: ${strategyName}`)
        } catch (error) {
          if (!error.message.includes('already registered')) {
            console.warn(`Failed to register custom locator strategy '${strategyName}':`, error)
          } else {
  ...
  async waitForText(text, sec = null, context = null) {
    ...
    if (context) {
      ...
      try {
        ...
        if (!locator.isXPath()) {
          return contextObject.locator(`${locator.simplify()} >> text=${text}`).first().waitFor({ timeout: waitTimeout, state: 'visible' })
        }
...
async function findElements(matcher, locator) {
  ...
  locator = new Locator(locator, 'css')
  this.debug(`FindElements called with locator:`, locator, `isCustom: ${locator.isCustom()}`)

  // Handle custom locators directly instead of relying on Playwright selector engines
  if (locator.isCustom()) {
    this.debug(`Processing custom locator: ${locator.type}=${locator.value}`)
...
async function findCustomElements(matcher, locator) {
  ...
  if (!strategyFunction) {
    this.debug(`Custom locator strategy "${locator.type}" not found`)
  ...
  this.debug(`Executing custom locator strategy: ${locator.type} with value: ${locator.value}`)
  ...
  this.debug(`Custom locator found ${elements.length} elements`)
