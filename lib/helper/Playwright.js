...
class Playwright extends Helper {
  ...
  *
  ...
  async startRecordingTraffic() {
    ...
    this.page.on('request', async (request) => {
      ...
      this.debugSection(`REQUEST: ${information}`);
  ...
  */
   ...
   */
  async seeTraffic(name, url, parameters, timeout = 10) {
    if (!this.recordedAtLeastOnce) {
    ...
    // the test will fail. Now the answer is because of mismatches or timeout
