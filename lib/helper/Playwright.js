...
class Playwright extends Helper {
  ...
  async seeInTitle(text) {
    return expect(this.page).toHaveTitle(text);
  }
  ...
  *
  ...
  async seeInCurrentUrl(url) {
    ...
    try {
      await expect(this.page).toHaveURL(_url);
    } catch (e) {
      if (/Timed out/i.test(e.message)) {
        throw new Error(`expected url to include ${url}, but found ${this.page.url()}`);
      } else {
        throw e;
      }
    }
  ...
  async waitInUrl(urlPart, sec = null) {
    ...
    try {
      await expect(this.page).toHaveURL(urlPart, { timeout: waitTimeout });
    } catch (e) {
      if (/Timed out/i.test(e.message)) {
        throw new Error(`expected url to include ${urlPart}, but found ${this.page.url()}`);
      } else {
        throw e;
      }
    }
