...
class Playwright extends Helper {
    ...
    try {
      ...
      for (const [strategyName, strategyFunction] of globalCustomLocatorStrategies.entries()) {
        if (!registeredCustomLocatorStrategies.has(strategyName)) {
          try {
            this.debugSection('[Custom Locator Strategies]', `Registering custom locator strategy: ${strategyName}`)
            ...
            const createCustomEngine = ((name, func) => {
              return () => {
                this.debugSection('[Custom Locator Strategies]', `Creating engine for ${name}`)
                return {
                  create(root, target) {
                    return null
                  },
                  query(root, selector) {
                    this.debugSection('[Custom Locator Strategies]', `Query called for ${name} with selector: ${selector}, root:`, root?.tagName)
                    try {
                      if (!root) return null
                      const result = func(selector, root)
                      this.debugSection('[Custom Locator Strategies]', `Query result for ${name}:`, result?.tagName || result)
                  ...
                  queryAll(root, selector) {
                    this.debugSection('[Custom Locator Strategies]', `QueryAll called for ${name} with selector: ${selector}, root:`, root?.tagName)
                    try {
                      if (!root) return []
                      const result = func(selector, root)
                      this.debugSection('[Custom Locator Strategies]', `QueryAll result for ${name}:`, result?.length || result?.tagName || result)
            ...
            this.debugSection('[Custom Locator Strategies]', `Successfully registered custom locator strategy: ${strategyName}`)
          } catch (error) {
            if (!error.message.includes('already registered')) {
              console.warn(`Failed to register custom locator strategy '${strategyName}':`, error)
            } else {
              this.debugSection('[Custom Locator Strategies]', `Custom locator strategy '${strategyName}' already registered`)
            }
          }
        }
      }
  ...
  async _before(test) {
    ...
    try {
      ...
      this.debugSection('[Custom Locator Strategies]', 'Failed to set metadata for reporting')
    }
  ...
  async _registerCustomLocatorStrategies() {
    ...
    for (const [strategyName, strategyFunction] of Object.entries(this.customLocatorStrategies)) {
      if (!registeredCustomLocatorStrategies.has(strategyName)) {
        try {
          this.debugSection('[Custom Locator Strategies]', `Registering custom locator strategy for context: ${strategyName}`)
          ...
          await playwright.selectors.register(strategyName, createCustomEngine)
          registeredCustomLocatorStrategies.add(strategyName)
          this.debugSection('[Custom Locator Strategies]', `Successfully registered custom locator strategy for context: ${strategyName}`)
        } catch (error) {
          if (!error.message.includes('already registered')) {
            console.warn(`Failed to register custom locator strategy '${strategyName}':`, error)
          } else {
  ...
  async amOnPage(url) {
    ...
    if (!/^\w+\:(\/\/|.+)/.test(url)) {
      ...
      this.debugSection('[Custom Locator Strategies]', `Changed URL to base url + relative path: ${url}`)
    }
  ...
  async _locate(locator) {
    ...
    if (store.debugMode) {
      ...
      if (els.length > 1) {
        ...
        this.debugSection('[Custom Locator Strategies]', `No elements found by ${JSON.stringify(locator).slice(0, 50)}....`)
      }
    }
  ...
  async closeOtherTabs() {
    ...
    if (otherPages.length) {
      this.debugSection('[Custom Locator Strategies]', `Closing ${otherPages.length} tabs`)
  ...
  async handleDownloads(fileName) {
    this.page.waitForEvent('download').then(async download => {
      ...
      this.debugSection('[Custom Locator Strategies]', 'Download completed')
  ...
  async grabTextFromAll(locator) {
    ...
    this.debugSection('[Custom Locator Strategies]', `Matched ${els.length} elements`)
    return texts
  }
  ...
  async saveElementScreenshot(locator, fileName) {
    ...
    assertElementExists(res, locator)
    const elem = res
    this.debugSection('[Custom Locator Strategies]', `Screenshot of ${new Locator(locator)} element has been saved to ${outputFile}`)
...
async function findElements(matcher, locator) {
  ...
  locator = new Locator(locator, 'css')
  this.debugSection('[Custom Locator Strategies]', `FindElements called with locator:`, locator, `isCustom: ${locator.isCustom()}`)

  // Handle custom locators directly instead of relying on Playwright selector engines
  if (locator.isCustom()) {
    this.debugSection('[Custom Locator Strategies]', `Processing custom locator: ${locator.type}=${locator.value}`)
  ...
  if (this.contextLocator) {
    const contextLocatorObj = new Locator(this.contextLocator, 'css')
    if (contextLocatorObj.isCustom()) {
      this.debugSection('[Custom Locator Strategies]', `Searching within custom context: ${contextLocatorObj.type}=${contextLocatorObj.value}`)
      // Find the context elements first
      const contextElements = await findCustomElements.call(this, matcher, contextLocatorObj)
      if (contextElements.length === 0) {
        this.debugSection('[Custom Locator Strategies]', `No context elements found`)
        return []
      }

      // Search within the first context element
      const locatorString = buildLocatorString(locator)
      this.debugSection('[Custom Locator Strategies]', `Using locator string within custom context: ${locatorString}`)
      return contextElements[0].locator(locatorString).all()
    }
  }

  const locatorString = buildLocatorString(locator)
  this.debugSection('[Custom Locator Strategies]', `Using locator string: ${locatorString}`)
...
async function findCustomElements(matcher, locator) {
  ...
  if (!strategyFunction) {
    this.debugSection('[Custom Locator Strategies]', `Custom locator strategy "${locator.type}" not found`)
  ...
  this.debugSection('[Custom Locator Strategies]', `Executing custom locator strategy: ${locator.type} with value: ${locator.value}`)
  ...
  this.debugSection('[Custom Locator Strategies]', `Custom locator found ${elements.length} elements`)
...
async function targetCreatedHandler(page) {
  ...
  if (this.options.windowSize && this.options.windowSize.indexOf('x') > 0 && this._getType() === 'Browser') {
    try {
      ...
      this.debugSection('[Custom Locator Strategies]', 'Target can be already closed, ignoring...')
    }
  }
}
