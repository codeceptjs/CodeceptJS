...
/**
 * Uses [Google Chrome's Playwright](https://github.com/GoogleChrome/Playwright) library to run tests inside headless Chrome.
 * Browser control is executed via DevTools Protocol (instead of Selenium).
 * This helper works with a browser out of the box with no additional tools required to install.
 *
 * Requires `Playwright` package to be installed.
 *
 * > Experimental Firefox support [can be activated](https://codecept.io/helpers/Playwright-firefox).
 *
 * ## Configuration
 *
 ...
 * * `browser`: (optional, default: chrome) - can be changed to `firefox` when using [Playwright-firefox](https://codecept.io/helpers/Playwright-firefox).
 * * `chrome`: (optional) pass additional [Playwright run options](https://github.com/GoogleChrome/Playwright/blob/master/docs/api.md#Playwrightlaunchoptions).
 *
 *
...
class Playwright extends Helper {
  ...
  async _startBrowser() {
    if (this.isRemoteBrowser) {
      try {
        this.browser = await playwright.connect(this.playwrightOptions);
      } catch (err) {
  ...
  async _evaluateHandeInContext(...args) {
    ...
    if (context.constructor.name === 'Frame') {
      ...
      // https://github.com/GoogleChrome/Playwright/issues/1051
  ...
  async switchToNextTab(num = 1) {
    const pages = await _getPages(this.browser);
  ...
  async waitForVisible(locator, sec) {
    ...
    locator = new Locator(locator, 'css');
    const context = await this._getContext();
    const waiter = context.waitForSelector(`${locator.type}=${locator.value}`, { timeout: waitTimeout, visible: true });
    return waiter.catch((err) => {
  ...
  async waitForInvisible(locator, sec) {
    ...
    locator = new Locator(locator, 'css');
    const context = await this._getContext();
    const waiter = context.waitForSelector(`${locator.type}=${locator.value}`, { timeout: waitTimeout, hidden: true });
    return waiter.catch((err) => {
  ...
  async waitToHide(locator, sec) {
    ...
    locator = new Locator(locator, 'css');
    const context = await this._getContext();
    return context.waitForSelector(`${locator.type}=${locator.value}`, { timeout: waitTimeout, hidden: true }).catch((err) => {
...
async function _getPages(browser) {
  const bc = await browser.browserContexts();
  let pages = [];

  bc.forEach(context => {
    if (context._delegate.existingPages().length > 0) {
      pages = context._delegate.existingPages();
    }
  });
