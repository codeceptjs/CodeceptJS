...
class Playwright extends Helper {
  ...
  async _after() {
    ...
    // close other sessions with timeout protection
    try {
      if ((await this.browser)?._type === 'Browser') {
        const contexts = await Promise.race([
          this.browser.contexts(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Get contexts timeout')), 3000))
        ])
        ...
        await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('Close contexts timeout')), 5000))
        ])
      }
    } catch (e) {
  ...
  async _afterSuite() {
    ...
    if (this.isRunning) {
      try {
        await Promise.race([
          this._stopBrowser(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Browser stop timeout')), 15000))
        ])
      } catch (e) {
  ...
  async _finishTest() {
    if ((restartsSession() || restartsContext()) && this.isRunning) {
      try {
        await Promise.race([
          this._stopBrowser(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Test finish timeout')), 10000))
        ])
      } catch (e) {
        ...
        if (this.isElectron) {
          ...
          try {
            ...
            browserContext = await Promise.race([
              this.browser.newContext(Object.assign(this.contextOptions, config)),
              new Promise((_, reject) => setTimeout(() => reject(new Error('New context timeout')), 10000))
            ])
            page = await Promise.race([
              browserContext.newPage(),
              new Promise((_, reject) => setTimeout(() => reject(new Error('New page timeout')), 5000))
            ])
          } catch (e) {
  ...
  async amOnPage(url) {
    ...
    try {
      ...
      if (this.browserContext) {
        ...
        try {
          await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('Context check timeout')), 1000))
          ])
        } catch (contextError) {
      ...
      if (err.message && (err.message.includes('ERR_ABORTED') || err.message.includes('frame was detached') || err.message.includes('Target page, context or browser has been closed') || err.message.includes('Cannot navigate'))) {
  ...
  async saveScreenshot(fileName, fullPage) {
    ...
    try {
      ...
      await Promise.race([
        this.page.url(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Page URL check timeout')), 1000))
      ])
  ...
  async switchTo(locator) {
      ...
      if (locator >= 0 && locator < childFrames.length) {
        try {
          this.context = await Promise.race([
            this.page.frameLocator('iframe').nth(locator),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Frame locator timeout')), 5000))
          ])
          this.contextLocator = locator
        } catch (e) {
    ...
    try {
      frame = await Promise.race([
        this._locateElement(locator),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Locate frame timeout')), 5000))
      ])
    } catch (e) {
    ...
    try {
      if (this.frame) {
        this.frame = await Promise.race([
          this.frame.frameLocator(locator),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Child frame locator timeout')), 5000))
        ])
      } else {
        this.frame = await Promise.race([
          this.page.frameLocator(locator),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Page frame locator timeout')), 5000))
        ])
      }
    } catch (e) {
...
async function refreshContextSession() {
  ...
  try {
    ...
    if (contextsToClose.length > 0) {
      await Promise.race([
        Promise.all(contextsToClose.map(c => c.close())),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Close contexts timeout')), 5000))
      ])
    }
  } catch (e) {
