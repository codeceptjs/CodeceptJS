...
class Playwright extends Helper {
  ...
  async _failed(test) {
    ...
    if (this.options.recordVideo && this.page && this.page.video()) {
      const videoPath = `${global.output_dir}${pathSeparator}videos${pathSeparator}${clearString(test.title)}.failed.webm`;
      test.artifacts.video = await this.page.video().path();
      this.page.video().saveAs(videoPath).then(() => {
         fs.renameSync(test.artifacts.video, videoPath, (() => {
  ...
  async _passed(test) {
    if (this.options.recordVideo && this.page && this.page.video()) {
      const videoPath = `${global.output_dir}${pathSeparator}videos${pathSeparator}${clearString(test.title)}.passed.webm`;
