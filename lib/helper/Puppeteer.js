...
/**
 * Class to handle the interaction with the Dialog Class from Puppeteer
 */
class DialogStore {
  constructor(dialog, defaultAction) {
    this._dialog = dialog || {};
    this._actionType = '';
  ...
  _assertValidActionType(action) {
    if (['accept', 'dismiss'].indexOf(action) === -1) {
      throw new Error('Invalid Dialog action type. Only "accept" or "dismiss" actions are accepted');
    }
  }
  ...
  set dialog(dialog) {
    if (this._dialog) {
      console.error('Dialog already exists and was not closed. Popups must always be closed by calling either I.acceptPopup() or I.cancelPopup()');
    }
    this._dialog = dialog;
  }
  ...
  assertPopupVisible() {
    if (!this._dialog) {
      throw new Error('Popup is not visible');
    }
  }
  ...
  assertPopupActionType(type) {
    ...
    if (expectedAction !== type) {
      throw new Error(`Dialog action does not fit the expected action type. Expected popup action to be '${expectedAction}' not '${type}`);
    }
    this.clear();
  }
}
...
class Puppeteer extends Helper {
  constructor(config) {
    ...
    dialogStore.defaultAction = this.options.defaultPopupAction;
  }
   ...
   */
  acceptPopup() {
    dialogStore.assertPopupActionType('accept');
  }
   ...
   */
  amCancellingPopups() {
    dialogStore.actionType = 'dismiss';
  }
   ...
   */
  cancelPopup() {
    dialogStore.assertPopupActionType('dismiss');
  }
  ...
  async seeInPopup(text) {
    dialogStore.assertPopupVisible();
    const popupText = await dialogStore.dialog.message();
   ...
   * @page {Puppeteer.Page}
   *
   * The dialog listener handles the dialog with the predefined action when it appears on the page.
   * It also saves a reference to the object which is used in seeInPopup.
   */
  _addDialogListener(page) {
    if (!page) {
      return;
    }
    page.on('dialog', async (dialog) => {
      dialogStore.dialog = dialog;
      const action = dialogStore.actionType || this.options.defaultPopupAction;
      ...
      switch (action) {
        ...
        default: {
          throw new Error('Unknown popup action type. Only "accept" or "dimiss" are accepted');
        }
      }
    });
  }
  ...
  /**
   * Grab the text within the popup.
   *
   * Example:
  ...
  grabPopupText() {
    dialogStore.assertPopupVisible();
    return dialogStore.dialog.message();
  }
