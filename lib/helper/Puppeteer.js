...
class Puppeteer extends Helper {
  ...
  async mock(method, oneOrMoreUrls, dataOrStatusCode) {
    const handler = await getPollyRouteHandler(this.polly, method, oneOrMoreUrls, this.options.url);

    if (typeof dataOrStatusCode === 'number') {
      const statusCode = dataOrStatusCode;
      handler.intercept((_, res) => {
        console.log("it's working");
        return res.sendStatus(statusCode);
      });
    } else {
      const customValue = dataOrStatusCode;
      handler.intercept((_, res) => res.send(customValue));
    }
  }
...
function checkIfMockingStarted(polly) {
  if (!polly) {
    throw new Error('Please start Mocking before using mock');
  }
}

async function getPollyRouteHandler(polly, method, oneOrMoreUrls, baseUrl) {
  checkIfMockingStarted(polly);
  const { server } = polly;

  method = method.toLowerCase();
  const serverHosted = await new Promise((resolve, reject) => {
    server.host(baseUrl, () => {
      resolve();
    });
  });
  ...
  switch (method) {
    case 'get':
      return server.get(oneOrMoreUrls);
    case 'put':
      return server.put(oneOrMoreUrls);
    case 'post':
      return server.post(oneOrMoreUrls);
    case 'patch':
      return server.patch(oneOrMoreUrls);
    case 'delete':
      return server.delete(oneOrMoreUrls);
    case 'merge':
      return server.merge(oneOrMoreUrls);
    case 'head':
      return server.head(oneOrMoreUrls);
    case 'options':
      return server.options(oneOrMoreUrls);
    default:
