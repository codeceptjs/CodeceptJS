...
class Puppeteer extends Helper {
   ...
   */
  async calculateLighthouseScores(htmlReport = false) {
    this.puppeteerOptions.port = this.browser._defaultContext._connection._url.split('/')[2].split(':')[1];
    await puppeteer.connect({ browserWSEndpoint: this.browser._defaultContext._connection._url });

    const url = await this._getPageUrl() || this.config.url;


    const report = await lighthouse(url, this.puppeteerOptions, null);

    if (htmlReport) {
      return reportGenerator.generateReport(report.lhr, 'html');
    }

    return `Lighthouse scores of Performance, Accessibility, Best Practices and SEO accordingly: ${Object.values(report.lhr.categories).map(c => c.score).join(', ')}`;
  }
}
