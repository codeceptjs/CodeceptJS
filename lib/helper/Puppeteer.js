...
class Puppeteer extends Helper {
  ...
  /**
   * Remove the Puppeteer.Dialog object from the store
   */
  _removeDialog() {
    pageStore.dialog = null;
  }

  acceptPopup() {
    this._assertPopupVisible();
    this._removeDialog(); // Dummy accept
  }

  cancelPopup() {
    this._assertPopupVisible();
    this._removeDialog(); // Dummy dismiss
  }

  async seeInPopup(text) {
    this._assertPopupVisible();
    const popupText = await pageStore.dialog.message();
    console.log('Checking popup', popupText);
    stringIncludes('text in popup').assert(text, popupText);
  }

  _setPage(page) {
    this._initPageListener(page);
    this.page = page;
  }

  setPopupHandlerAction(actionType) {
    if (['accept', 'dismiss'].indexOf(actionType) === -1) {
      throw new Error('Invalid popup action type. Only accept or dismiss can be defined');
    }
    pageStore.dialogAction = actionType;
  }
   ...
   */
  _initPageListener(page) {
    if (!page) {
      return;
    }
    page.on('dialog', async (dialog) => {
      if (pageStore.dialog) {
        console.error('Dialog already exists and was not closed. Please always close the dialog boxes by using the I.acceptPopup() or I.cancelPopup()');
      }
      pageStore.dialog = dialog;

      if (pageStore.dialogAction === 'accept') {
