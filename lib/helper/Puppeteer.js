...
class Puppeteer extends Helper {
  ...
  async waitInUrl(urlPart, sec = null) {
    ...
    return this.page.waitForFunction((urlPart, decodeUrl) => {
      eval(decodeUrl); // eslint-disable-line no-eval
      const currUrl = decodeUrl(window.location.href);
      return currUrl.indexOf(urlPart) > -1;
    }, { timeout: waitTimeout }, urlPart, decodeUrl.toString()).catch(async (e) => {
...
async function proceedSeeInField(assertType, field, value) {
  ...
  if (tag === 'SELECT') {
    ...
    if (value === '') {
      const option = await el.$$('option:checked');
      return equals(`select option by ${field}`)[assertType](1, option.length);
    }

    let options = await el.$$(`option[value="${value}"]`);
    options = await filterFieldsByValue(options, value, true);
