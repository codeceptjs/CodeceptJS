...
class Puppeteer extends Helper {
  ...
  async waitForClickable(locator, waitTimeout) {
    const _locator = new Locator(locator, 'css');
    const els = await this._locate(_locator);
    assertElementExists(els, _locator);

    return this.waitForFunction(isElementClickable, [els[0]], waitTimeout).catch(async (e) => {
      if (/failed: timeout/i.test(e.message)) {
        throw new Error(`element ${_locator.toString()} still not clickable after ${waitTimeout || this.options.waitForTimeout / 1000} sec`);
      } else {
        throw e;
      }
    });
