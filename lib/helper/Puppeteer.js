 ...
 * * `basicAuth`: (optional) the basic authentication to pass to base url. Example: 'username, password'
...
class Puppeteer extends Helper {
  ...
  async amOnPage(url) {
    ...
    if (this.config.basicAuth) {
      const username = this.config.basicAuth.split(',')[0].trim();
      const pwd = this.config.basicAuth.split(',')[1].trim();
      this.page.authenticate({ username, password: pwd });
    }
