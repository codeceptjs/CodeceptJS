...
class Puppeteer extends Helper {
  ...
  async waitForEnabled(locator, sec) {
    ...
    if (locator.isCSS()) {
      const enabledFn = function (locator) {
        ...
        return Array.from(els).filter(el => !el.disabled).length > 0;
      };
  ...
  async waitForValue(locator, value, sec) {
    ...
    if (locator.isCSS()) {
      const valueFn = function (locator, value) {
        ...
        return Array.from(els).filter(el => (el.value || '').indexOf(value) !== -1).length > 0;
      };
  ...
  async waitNumberOfVisibleElements(locator, num, sec) {
    ...
    if (locator.isCSS()) {
      const visibleFn = function (locator, num) {
        ...
        return Array.from(els).filter(el => el.offsetParent !== null).length === num;
      };
  ...
  async waitForElement(locator, sec) {
    ...
    const context = await this._getContext();
    const waiter = context.waitForSelector(locator.value, { timeout: waitTimeout });
    return waiter.catch((err) => {
  ...
  async waitForVisible(locator, sec) {
    ...
    const matcher = await this.context;
    const context = await this._getContext();
    const waiter = context.waitForSelector(locator.value, { timeout: waitTimeout, visible: true });
    return waiter.catch((err) => {
  ...
  async waitForInvisible(locator, sec) {
    ...
    const matcher = await this.context;
    const context = await this._getContext();
    const waiter = context.waitForSelector(locator.simplify(), { timeout: waitTimeout, hidden: true });
    return waiter.catch((err) => {
