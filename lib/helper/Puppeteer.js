...
class Puppeteer extends Helper {
  ...
  getProp(prop) {
    if (prop) {
      console.log(this.context._page[prop]);
      return;
    }
  ...
  async _withinBegin(locator) {
    ...
    console.log('Frame selector', locator);
    console.log('Current Page', this.page);

    // const frames = await this.page.frames();
    const frames = this.page.frames();

    const frameName = 'content';

    const runnerFrame = await findFrame(this.page, frameName);

    if (runnerFrame) {
      const customLocator = new Locator('rus', 'css');
      console.log('rus xpath', customLocator.value);
      const rusSel = ".//*[self::input | self::textarea | self::select][not(./@type = 'submit' or ./@type = 'image' or ./@type = 'hidden')][((./@name = 'rus') or ./@id = //label[normalize-space(string(.)) = 'rus']/@for or ./@placeholder = 'rus')] | .//label[normalize-space(string(.)) = 'rus']//.//*[self::input | self::textarea | self::select][not(./@type = 'submit' or ./@type = 'image' or ./@type = 'hidden')]";
      // const elements = await runnerFrame.$x(rusSel, runnerFrame);
      const elements = await runnerFrame.$x(rusSel);
      console.log('Found rus elements:', elements.length);

      if (elements.length) {
        console.log('Found rus 1st element:', elements[0].constructor.name);
        console.log('Found rus 1st element:', elements[0].asElement());
      }
    }

    // console.log('Current Frames', frames);
    const els = await this._locate(locator);
    assertElementExists(els, locator);
    this.context = els[0];

    if (runnerFrame) {
      this.context = runnerFrame;
    }
    console.log('Context Type:', this.context && this.context.constructor && this.context.constructor.name);
    console.log('runnerFrame Type:', runnerFrame && runnerFrame.constructor && runnerFrame.constructor.name);
  ...
  async _locate(locator) {
    ...
    // return debugFindElements(await this.context, locator);
  }
  ...
  async fillField(field, value) {
    ...
    const el = els[0];
    // await el.focus();
    const el2 = await this.context.evaluate(body => body, el);

    const el3 = await debugGetElement(el);

    console.log('getting properties on: ', await el3.getProperty('tagName').then(el => el.jsonValue()));
    // const log22 = await el2.getProperty('tagName');
    const tag = await el.getProperty('tagName').then(el => el.jsonValue());
    const editable = await el.getProperty('contenteditable').then(el => el.jsonValue());

    console.log('element:', { tag, editable });
    if (tag === 'TEXTAREA' || editable) {
      // await context.evaluateHandle(el => el.innerHTML = '', el);
      await this._evaluateHandeInContext(el => el.innerHTML = '', el);
      // await this._evaluateHandeInContext(el => el.innerHTML = '', el);
    }
    if (tag === 'INPUT') {
      await this._evaluateHandeInContext(el => el.value = '', el);
      // await context.evaluateHandle(el => el.value = '', el);
    }
  ...
  async waitForElement(locator, sec) {
    ...
    let waiter;
    if (locator.isCSS()) {
      waiter = this._getContext().waitForSelector(locator.simplify(), { timeout: waitTimeout });
    } else {
      waiter = this._getContext().waitForFunction($XPath, { timeout: waitTimeout }, null, locator.value);
    }
  ...
  async waitForVisible(locator, sec) {
    ...
    let waiter;
    if (locator.isCSS()) {
      waiter = this._getContext().waitForSelector(locator.simplify(), { timeout: waitTimeout, visible: true });
    } else {
      const visibleFn = function (locator, $XPath) {
        eval($XPath); // eslint-disable-line no-eval
        return $XPath(null, locator).filter(el => el.offsetParent !== null).length > 0;
      };
      waiter = this._getContext().waitForFunction(visibleFn, { timeout: waitTimeout }, locator.value, $XPath.toString());
    }
    return waiter.catch((err) => {
  ...
  async waitToHide(locator, sec) {
    ...
    let waiter;
    if (locator.isCSS()) {
      waiter = this._getContext().waitForSelector(locator.simplify(), { timeout: waitTimeout, hidden: true });
    } else {
      const visibleFn = function (locator, $XPath) {
        eval($XPath); // eslint-disable-line no-eval
        return $XPath(null, locator).filter(el => el.offsetParent !== null).length === 0;
  ...
  async waitForText(text, sec = null, context = null) {
    ...
    const controlObject = await this._getContext();

    if (context) {
      const locator = new Locator(context, 'css');
      if (locator.isCSS()) {
        waiter = controlObject.waitForFunction((locator, text) => {
      ...
      if (locator.isXPath()) {
        waiter = controlObject.waitForFunction((locator, text, $XPath) => {
      ...
      waiter = controlObject.waitForFunction(text => document.body.innerText.indexOf(text) > -1, { timeout: waitTimeout }, text);
    }
  ...
  async waitUntil(fn, sec = null) {
    const aSec = sec || this.options.waitForTimeout;
    const waitTimeout = aSec * 1000;
    return this._getContext().waitForFunction(fn, { timeout: waitTimeout });
  }
  ...
  async waitUntilExists(locator, sec) {
    ...
    if (locator.isCSS()) {
      ...
      waiter = this._getContext().waitForFunction(visibleFn, { timeout: waitTimeout }, locator.value);
    } else {
      const visibleFn = function (locator, $XPath) {
        eval($XPath); // eslint-disable-line no-eval
...
async function findFrame(page, locator) {
  ...
  const matchingFrame = frames.find((frame) => {
    ...
    return frame.name() === locator;
  });
  return matchingFrame;
}

async function debugFindElements(matcher, locator) {
  locator = new Locator(locator, 'css');
  console.log('xpath:', locator.value);
  const arrayHandles = await matcher.$x(locator.value, matcher);
  console.log('debugFindElements:', arrayHandles.length);

  const results = [];
  for (const handle of arrayHandles) {
    console.log('handle type:', handle.constructor.name);
    results.push(handle.asElement());
  }

  return results;
}

async function debugGetElement(matcher, locator) {
  return matcher.asElement();
}

async function findElements(matcher, locator) {
  locator = new Locator(locator, 'css');
  if (!locator.isXPath()) {
    console.log('Not XPath selector');
    return matcher.$$(locator.simplify());
  }

  console.log('Is Xpath locator');

  let context = null;
  console.log('matcher type:', matcher.constructor.name);
  ...
  // const arrayHandle = await matcher.$x()
  console.log('locator value:', locator.value, context);
  const arrayHandles = await matcher.$x(locator.value, matcher);

  console.log('findElements: ', arrayHandles.length);
  return arrayHandles;

  const arrayHandle = Array.isArray(arrayHandles) && arrayHandles[0];

  // const arrayHandle = await matcher.executionContext().evaluateHandle($XPath, context, locator.value);
  const properties = await arrayHandle.getProperties();

  console.log('properties:', [...properties.values()].length);

  const result = [];

  result.push(arrayHandle.asElement());
  return result;

  for (const el of arrayHandles) {
    const props = await arrayHandle.getProperties();
    console.log('properties:', props.length);
    // result.push(el.asElement());
    // el.dispose();
  }
  // await arrayHandle.dispose();
  return result;
  console.log('property values:', [...properties.values()].length);
