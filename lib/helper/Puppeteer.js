...
class Puppeteer extends Helper {
  ...
  async switchTo(locator) {
    if (!locator) {
      ...
      // Select by frame index of current context (i.e. )
      const childFrames = this.context ? this.context.childFrames() : this.page.frames();

      if (locator >= 0 && locator < childFrames.length) {
        this.context = childFrames[locator];
      } else {
        throw new Error('Frame index does not exist');
      }
      return;
    }
...
async function findFrame(context, locator, element) {
  ...
  console.log('Looking for iframe:', locator);
...
async function findElements(matcher, locator) {
  ...
  const result = await matcher.$x(locator.value, matcher);

  console.log('findElements: ', result.length);
  return result;
}
...
async function proceedSee(assertType, text, context) {
  ...
  let allText;
  if (!context) {
    // console.log('no context', await this.context);
    let el = await this.context;
    // Use body of either the frame or page
