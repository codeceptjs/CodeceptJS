...
class Puppeteer extends Helper {
  ...
  async checkOption(field, context = null) {
    const elm = await this._getFirstCheckableElm(field, context);
  ...
  /**
   * Get the first checkable DOM element
   *
   * @param field the field
   * @param context the context (optional)
   * @returns {Promise<*>} that resolves with the checkable DOM element
   * @private
   */
  async _getFirstCheckableElm(field, context = null) {
   ...
   *
   * @param elm the DOM element to click on
   * @returns {Promise<*>} delegating to waitForAction()
   * @private
   */
  async _clickElm(elm) {
    await elm.click();
