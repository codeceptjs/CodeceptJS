 ...
 *
 * ```js
 * I.mutate('createUser', { login: 'davert', email: 'davert@mail.com' });
 * let user = await I.mutate('createUser', { name: 'davert'});
 ...
 *
 * * `operation`: The operation/mutation that needs to be performed for creating a record in the backend DB.
 *
 ...
 *    as the 'variables' object to go along with the GQL query.
 * * `revert`: A function called with the data returned when an item is created. The object returned by
 *    this function is will be used to later delete the items created. So, make sure ID IS RETURNED when
 *    a record is created by a mutation.
 *
 * ### Requests
 *
 ...
 * ### Responses
 *
 * By default `I.mutate()` returns a promise with created data as specified in operation query string:
 *
 * ```js
 * let client = await I.mutate('createClient');
 * ```
 *
