...
class GraphQLDataFactory extends Helper {
  ...
  have(factory, params) {
    ...
    const createOperation = this.factories[factory].operations;
  ...
  _requestCreate(factory, variables) {
    ...
    return this.graphqlHelper.sendMutation(operation).then((resp) => {
      const id = this._fetchId(resp.data.data, factory, 'create');
  ...
  _requestDelete(factory, id) {
    ...
    return this.graphqlHelper._executeQuery(request).then((response) => {
