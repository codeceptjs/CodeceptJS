 ...
 *            variablesObjectGenerator: (data) => ({ id: data.id }),
 *          },
 *        }
 *      }
 *    }
...
class GraphQLDataFactory extends Helper {
   ...
   * ```
   *
   * @param {*} mutationName to use
   * @param {*} params predefined parameters
   */
  mutate(mutationName, params) {
    const variables = this._createItem(mutationName, params);
    this.debug(`Creating ${mutationName} ${JSON.stringify(variables)}`);
    return this._requestCreate(mutationName, variables);
  }
  ...
  haveMultiple(factory, times, params) {
    ...
    for (let i = 0; i < times; i++) {
      promises.push(this.mutate(factory, params));
    }
    return Promise.all(promises);
  }

  _createItem(mutation, data) {
    if (!this.factories[mutation]) {
      throw new Error(`Mutation ${mutation} is not defined in config.factories`);
    }
    let modulePath = this.factories[mutation].factory;
    try {
      try {
   ...
   *
   * @param {*} mutation to be used
   * @param {*} variables to be sent along with the query
   */
  _requestCreate(mutation, variables) {
    const { query } = this.factories[mutation];
    return this.graphqlHelper.sendMutation(query, variables).then((response) => {
      ...
      const data = response.data.data[mutation];
      this.created[mutation].push(data);
   ...
   */
  _requestDelete(mutation, data) {
    const deleteMutationObject = this.factories[mutation].revert(data);
    const { query, variables } = deleteMutationObject;

    return this.graphqlHelper.sendMutation(query, variables)
      .then((response) => {
        const idx = this.created[mutation].indexOf(data);
        this.debugSection('Deleted Id', `Id: ${data.id || data._id}`);
        this.created[mutation].splice(idx, 1);
      });
  }
}
