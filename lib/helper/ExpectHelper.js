...
class ExpectHelper {
    ...
    expectStartsWith(actualValue, expectedValueToStartWith, customErrorMsg = '') {
        // @ts-ignore
        output.step(
            `I expect "${JSON.stringify(actualValue)}" to start with "${JSON.stringify(expectedValueToStartWith)}"`,
        )
    ...
    expectJsonSchema(targetData, jsonSchema, customErrorMsg = '') {
        // @ts-ignore
        output.step(
            `I expect "${JSON.stringify(targetData)}" to match this JSON schema "${JSON.stringify(jsonSchema)}"`,
        )
    ...
    expectLengthAboveThan(targetData, lengthAboveThan, customErrorMsg = '') {
        // @ts-ignore
        output.step(
            `I expect "${JSON.stringify(targetData)}" to have length of above ${JSON.stringify(lengthAboveThan)}`,
        )
    ...
    expectLengthBelowThan(targetData, lengthBelowThan, customErrorMsg = '') {
        // @ts-ignore
        output.step(
            `I expect "${JSON.stringify(targetData)}" to have length of below ${JSON.stringify(lengthBelowThan)}`,
        )
    ...
    expectEqualIgnoreCase(actualValue, expectedValue, customErrorMsg = '') {
        // @ts-ignore
        output.step(
            `I expect and ingore case "${JSON.stringify(actualValue)}" to equal "${JSON.stringify(expectedValue)}"`,
        )
