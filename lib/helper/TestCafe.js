...
class TestCafe extends Helper {
  ...
  async dontSee(text, context = null) {
    ...
    return this.t
      .expect(els.filterVisible().count).toEqual(0, `Element with text "${text}" can still be seen`)
      .catch(mapError);
  }
  ...
  async seeInField(field, value) {
    ...
    return this.t
      .expect(await el.value).toEqual(value)
      .catch(mapError);
  }
  ...
  async waitNumberOfVisibleElements(locator, num, sec) {
    ...
    return this.t
      ...
      .toEqual(num, `The number of elements (${(new Locator(locator))}) is not ${num} after ${sec} sec`, { timeout: waitTimeout })
      .catch(mapError);
  }
