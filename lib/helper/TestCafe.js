...
class TestCafe extends Helper {
  ...
  async _startBrowser(param) {
    this.dummyTestcafeFile = createTestFile(global.output_dir, param.title || `test ${param.id}`); // create a dummy test file to get hold of the test controller
    ...
    // const logger = await RequestLogger();
    //
    // this.dummyTestcafeFile.requestHooks(logger);
  ...
  async _before(param) {
    if (this.options.restart && !this.options.manualStart) return this._startBrowser(param);
    if (!this.isRunning && !this.options.manualStart) return this._startBrowser(param);
    this.context = null;
  }
