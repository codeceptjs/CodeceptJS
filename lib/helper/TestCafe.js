...
class TestCafe extends Helper {
  ...
  async _configureAndStartBrowser(param) {
  ...
  async _before(param) {
    if (this.options.restart && !this.options.manualStart) return this._configureAndStartBrowser(param);
    if (!this.isRunning && !this.options.manualStart) return this._configureAndStartBrowser(param);
    this.context = null;
  }
