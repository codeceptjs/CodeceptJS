...
class WebDriverIO extends Helper {
  ...
  seeElementInDOM(locator) {
    return this.browser.elements(withStrictLocator(locator)).then(res => empty('elements').negate(res.value));
  }
  ...
  dontSeeElementInDOM(locator) {
    return this.browser.elements(withStrictLocator(locator)).then(res => empty('elements').assert(res.value));
  }
  ...
  waitForEnabled(locator, sec = null) {
    ...
    return client.waitUntil(() => client.elements(withStrictLocator(locator)).then((res) => {
      if (!res.value || res.value.length === 0) {
        return false;
      }
      const commands = [];
  ...
  waitForText(text, sec = null, aContext = null) {
    ...
    return client.waitUntil(
      () => client.elements(withStrictLocator(context)).then((res) => {
        if (!res.value || res.value.length === 0) {
          return false;
        }
        const commands = [];
  ...
  waitNumberOfVisibleElements(locator, num, sec = null) {
    const client = this.browser;
    const aSec = sec || this.options.waitForTimeout;
    return client.waitUntil(() => client.elements(withStrictLocator(locator)).then(function (res) {
      if (!res.value || res.value.length === 0) {
        return false;
      }
      const commands = [];
