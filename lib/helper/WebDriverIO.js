...
class WebDriverIO extends Helper {
   ...
   * ```js
   * I.pressCursorDown('left');
   * I.pressCursorDown('middle');
   * I.pressCursorDown('right');
   * ````
   */
  pressCursorDown(buttonName) {
    buttonName = buttonName.toLowerCase();
    const button = { 'left': 0, 'middle': 1, 'right': 2 };
    if (typeof button[buttonName] === 'undefined')
        throw new Error(`Button ${buttonName.toString()} was invalid`);
    return this.browser.buttonDown(button[buttonName]);
  }
  ...
  * ```js
  * I.pressCursorUp('left');
  * I.pressCursorUp('middle');
  * I.pressCursorUp('right');
  * ```
  */
  pressCursorUp(buttonName) {
    buttonName = buttonName.toLowerCase();
    const button = { 'left': 0, 'middle': 1, 'right': 2 };
    if (typeof button[buttonName] === 'undefined')
        throw new Error(`Button ${buttonName.toString()} was invalid`);
    return this.browser.buttonUp(button[buttonName]);
  }

  /**
  * Drag an item to a destination element.
  * ```js
  * I.dragAndDropElementTo('#dragHandle', '#container');
  * ```
  */
  dragAndDropElementTo(srcElement, destElement) {
