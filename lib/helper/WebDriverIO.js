...
class WebDriverIO extends Helper {
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    let values = Object.keys(attributes).map(key => attributes[key]);
  ...
  async saveScreenshot(fileName, fullPage = false) {
    ...
    let { width, height } = await this.browser.execute(() => ({
  ...
  async resizeWindow(width, height) {
    ...
    let { screenWidth, screenHeight } = await this.browser.execute(() => ({
  ...
  async waitForEnabled(locator, sec = null) {
    ...
    return client.waitUntil(async () => {
      ...
      let selected = await client.unify(commands, { extractValue: true });
  ...
  async waitForText(text, sec = null, aContext = null) {
    ...
    return this.browser.waitUntil(async () => {
      let res = await this.browser.elements(withStrictLocator(context));
...
async function proceedSeeField(assertType, field, value) {
  ...
  let proceedSingle = el => this.browser.elementIdAttribute(el.ELEMENT, 'value').then(res => stringIncludes(`fields by ${field}`)[assertType](value, res.value));
