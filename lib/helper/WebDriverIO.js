...
class WebDriverIO extends Helper {
  ...
  _after() {
    ...
    return this.browser.execute('localStorage.clear();').then(() => {
      return this.browser.deleteCookie().then(() => {
      return this.browser.waitUntil(function () {
              return this.browser.getTabIds().then(function (handles) {
                for (var i = 0; i < handles.length - 1; i++) {
                  this.browser.close(handles[i]);
                }
                return this.browser.switchTab(handles[0]);
              });
          }, 30000);
        });
    });
