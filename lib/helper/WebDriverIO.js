...
class WebDriverIO extends Helper {
  ...
  async selectOption(select, option) {
    ...
    let els = await asyncForEach(option, async opt => this.browser.elementIdElements(elem.ELEMENT, Locator.select.byVisibleText(xpathLocator.literal(opt))));
    ...
    if (Array.isArray(els) && els.length) {
      return asyncForEach(els, clickOptionFn);
    }
    // select options by value
    els = await asyncForEach(option, async opt => this.browser.elementIdElements(elem.ELEMENT, Locator.select.byValue(xpathLocator.literal(opt))));
    if (els.length === 0) {
  ...
  async grabTextFrom(locator) {
    ...
    return asyncForEach(res.value, async el => this.browser.elementIdText(el.ELEMENT));
  }
  ...
  async grabValueFrom(locator) {
    ...
    return asyncForEach(res.value, async el => this.browser.elementIdAttribute(el.ELEMENT, 'value'));
  }
  ...
  async grabCssPropertyFrom(locator, cssProperty) {
    const res = await this._locate(locator, true);
    assertElementExists(res, locator);
    return asyncForEach(res.value, async el => this.browser.elementIdCssProperty(el.ELEMENT, cssProperty));
  }
  ...
  async grabAttributeFrom(locator, attr) {
    const res = await this._locate(locator, true);
    assertElementExists(res, locator);
    return asyncForEach(res.value, async el => this.browser.elementIdAttribute(el.ELEMENT, attr));
  }
  ...
  async seeElement(locator) {
    ...
    const selected = await asyncForEach(res.value, async el => this.browser.elementIdDisplayed(el.ELEMENT));
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    let props = await asyncForEach(res.value, async (el) => {
      return asyncForEach(Object.keys(cssProperties), async prop => this.browser.elementIdCssProperty(el.ELEMENT, prop));
    });
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    let attrs = await asyncForEach(res.value, async (el) => {
      return asyncForEach(Object.keys(attributes), async attr => this.browser.elementIdAttribute(el.ELEMENT, attr));
    });
  ...
  async grabNumberOfVisibleElements(locator) {
    ...
    let selected = await asyncForEach(res.value, async el => this.browser.elementIdDisplayed(el.ELEMENT));
  ...
  async waitForEnabled(locator, sec = null) {
    ...
    return client.waitUntil(async () => {
      ...
      const selected = await asyncForEach(res.value, async el => client.elementIdEnabled(el.ELEMENT));
  ...
  async waitForText(text, sec = null, aContext = null) {
    ...
    return this.browser.waitUntil(
      async () => {
        ...
        const selected = await asyncForEach(res.value, async el => this.browser.elementIdText(el.ELEMENT));
        if (Array.isArray(selected)) {
  ...
  async waitForValue(field, value, sec = null) {
    ...
    return client.waitUntil(
      async () => {
        const res = await findFields.call(this, field);
        if (!res.value || res.value.length === 0) return false;
        const selected = await asyncForEach(res.value, async el => this.browser.elementIdAttribute(el.ELEMENT, 'value'));
        if (Array.isArray(selected)) {
...
async function asyncForEach(array, callback, option = {}) {
  const {
    extractValue = true,
  ...
  for (let index = 0; index < inputArray.length; index++) {
    ...
    try {
      ...
      if (Array.isArray(res) && expandArrayResults) {
        console.log('Expanding array results');
 ...
 * @param  {Object} [option]    if true extract value property from selenium result
 */
function unify(promises, option = {}) {
  const { extractValue = false, lowercase = false } = option;

  let result = Array.isArray(promises) ? promises : [promises];
...
async function proceedSeeField(assertType, field, value) {
  ...
  const proceedMultiple = async (fields) => {
    try {
      // await asyncForEach(fields, async el => this.browser.elementIdSelected(el.ELEMENT));
    } catch (err) {
      throw err;
    }

    const fieldResults = toArray(await asyncForEach(fields, async (el) => {
    ...
    if (typeof value === 'boolean') {
      ...
      if (assertType === 'assert') {
        equals(`select option by ${field}`)[assertType](true, fieldResults.length > 0);
      }
  ...
  if (tag.value === 'input') {
    ...
    if (fieldType === 'checkbox' || fieldType === 'radio') {
      if (typeof value === 'boolean') {
        // Filter by values
        const options = [];
        await asyncForEach(res.value, async (el) => {
          const isSelected = unify(await this.browser.elementIdSelected(el.ELEMENT), { extractValue: true });
          if (isSelected) {
            options.push(el);
          }
...
async function proceedSeeCheckbox(assertType, field) {
  ...
  const selected = await asyncForEach(res.value, async el => this.browser.elementIdSelected(el.ELEMENT));
