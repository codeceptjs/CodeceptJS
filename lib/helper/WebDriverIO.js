...
class WebDriverIO extends Helper {
  ...
  /**
   * Return text from random element, if there is several elements with the same locator
   */
  grabTextFromRandomElement(locator) {
    let client = this.browser;
    return this._locate(locator).then((res) => {
      if (!res.value || res.value.length === 0) {
        throw new Error(`${locator} cant be located by name|text|CSS|XPath`);
  ...
  /**
   * Return attribute value from random element, if there is several elements with the same locator
   */
  grabAttributeFromRandomElement(locator, attr) {
    let client = this.browser;
    return this._locate(locator).then((res) => {
      if (!res.value || res.value.length === 0) {
        throw new Error(`${locator} cant be located by name|text|CSS|XPath`);
  ...
  seeNumberOfVisibleElements(locator, num) {
    return this.browser.elements(locator).then(function (res) {
      ...
      return this.unify(commands, {
        ...
        return assert.equal(selected.length, num,
          `expected number of visible elements (${locator}) is ${num}, but found ${selected.length}`);
      });
    });
  }

  /**
   * Checks that there is an Element with given CSS property.
   */
  seeCssPropertyOnElement(locator, cssProperty, value) {
    let client = this.browser;
    return this._locate(locator).then((res) => {
      if (!res.value || res.value.length === 0) {
        throw new Error(`${locator} cant be located by name|text|CSS|XPath`);
      ...
      return client.unify(commands, {
        ...
        return assert.ok(selected.length > 0,
          `Element ${locator} hasn't CSS property ${cssProperty} with value ${value}`);
      });
    });
  }

  /**
   * Checks that there is an Element with given attribute.
   */
  seeAttributeOnElement(locator, attr, value) {
    let client = this.browser;
    return this._locate(locator).then((res) => {
      if (!res.value || res.value.length === 0) {
        throw new Error(`${locator} cant be located by name|text|CSS|XPath`);
      ...
      return client.unify(commands, {
        ...
        if (!Array.isArray(selected)) selected = [selected];
        selected = selected.filter((val) => val === value);
        return assert.ok(selected.length > 0, `Element ${locator} hasn't attribute ${attr} with value ${value}`);
      });
    });
  }
  ...
  /**
   * Waiting for the part of the URL to match the expected
   */
   ...
   */
  waitForValue(value, context, sec = null) {
    let client = this.browser;
    sec = sec || this.options.waitForTimeout;
    return client.waitUntil(function () {
        return client.elements(withStrictLocator(context)).then(function (res) {
   ...
   */
  changeTab(num, amountOfTabs = null, sec = null) {
    sec = sec || this.options.waitForTimeout;
    amountOfTabs = amountOfTabs || num;
    let client = this.browser;
    return client
      .waitUntil(function () {
        return this.getTabIds().then(function (handles) {
          return handles.length === amountOfTabs;
        });
      }, sec * 1000, "The number of tabs hasn't changed to " + amountOfTabs)
