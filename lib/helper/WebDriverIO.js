...
class WebDriverIO extends Helper {
  ...
  constructor(config) {
    ...
    this.options.desiredCapabilities.browserName =
      this.options.browser || this.options.desiredCapabilities.browserName;
  ...
  grabTextFrom(locator) {
    ...
    return this._locate(locator).then((res) => {
      ...
      let elem = res.value[0];
      return client.elementIdText(elem.ELEMENT).then((res) => {
        return res.value;
      });
    });
  }
  ...
  grabValueFrom(locator) {
    ...
    return this._locate(locator).then((res) => {
      ...
      let elem = res.value[0];
      return client.elementIdAttribute(elem.ELEMENT, 'value').then((res) => {
        return res.value;
      });
    });
  }
  ...
  grabAttributeFrom(locator, attr) {
    ...
    return this._locate(locator).then((res) => {
      ...
      let elem = res.value[0];
      return client.elementIdAttribute(elem.ELEMENT, attr).then((res) => {
        return res.value;
      });
    });
  }
  ...
  seeNumberOfVisibleElements(selector, num) {
    return this.browser.elements(selector).then(function (res) {
      if (!res.value || res.value.length === 0) {
      ...
      return this.unify(commands, {
        ...
        return truth(`elements of ${selector}`, 'to be seen').assert.equal(selected.length, num);
      });
    });
  }
    ...
    if (locator) {
      return client.element(withStrictLocator(locator)).then(function (res) {
        ...
        return this.elementIdLocation(elem.ELEMENT).then(function (location) {
          if (!location.value || location.value.length === 0) throw new Error(`Failed to recieve (${srcElement}) location`);
    ...
    return client.element(withStrictLocator(locator)).then(function (res) {
      ...
      if (client.isMobile) {
        return this.elementIdSize(elem.ELEMENT).then(function (size) {
          ...
          return this.elementIdLocation(elem.ELEMENT).then(function (location) {
            if (!location.value || location.value.length === 0) throw new Error(`Failed to recieve (${locator}) location`);
        ...
        return this.elementIdLocation(elem.ELEMENT).then(function (location) {
          ...
          return this.touchDown(location.value.x, location.value.y).then(function (res) {
            ...
            return client.element(withStrictLocator(destElement)).then(function (res) {
              if (!res.value || res.value.length === 0) return truth(`elements of ${destElement}`, 'to be seen').assert(false);
              elem = res.value;
              return this.elementIdLocation(elem.ELEMENT).then(function (location) {
                if (!location.value || location.value.length === 0) throw new Error(`Failed to recieve (${destElement}) location`);
  ...
  switchTo(locator) {
    ...
    if (typeof locator === 'undefined') return this.browser.frame(null);
...
function proceedSee(assertType, text, context) {
  ...
  return this._locate(context).then((res) => {
    if (!res.value || res.value.length === 0) {
      throw new Error(`${locator} cant be located by name|text|CSS|XPath`);
    }
    let elem = res.value[0];
    return client.elementIdText(elem.ELEMENT).then((res) => {
      return stringIncludes(description)[assertType](text, res.value);
    });
  });
}
