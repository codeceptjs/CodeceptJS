...
class WebDriverIO extends Helper {
  ...
  _after() {
    ...
    if (this.options.keepCookies) return Promise.all(
  ...
  scrollTo(locator, offsetX, offsetY) {
    ...
    if (locator) {
      ...
      return client.execute(function (location) {
        return this.execute(function scroll(x, y) {
          return window.scrollTo(x, y);
        }, offsetX, offsetY);
      });
  ...
  dragAndDrop2(srcElement, destElement) {
  ...
  dragAndDrop(srcElement, destElement) {
    ...
    return this2.moveCursorTo(srcElement).then(function (res) {
      //validate res
      return this.buttonDown().then(function (res) {
        //validate result
        return this2.moveCursorTo(destElement).then(function (res) {
          //validate result
