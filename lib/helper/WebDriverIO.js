...
class WebDriverIO extends Helper {
   ...
   *
   * ```js
   * I.seeCssPropertyOnElement('h3', 'font-weight', "bold");
   * ```
   */
   ...
   *
   * ```js
   * I.seeAttributeOnElement('//form', 'method', "post");
   * ```
   */
      ...
      .waitUntil(function () {
        return this.getTabIds().then(function (handles) {
          return this.getCurrentTabId().then(function(current) {
            if (handles.indexOf(current) + num + 1 <= handles.length) return this.switchTab(handles[handles.indexOf(current) + num])
            else return false;
          });
        });
