...
class WebDriverIO extends Helper {
   ...
   * Appium: support Android and iOS
   { status: 0,
  value: [ 'NATIVE_APP' ],
  sessionId: '31183895-551b-4cc0-97be-79332860349e' }
   */
  ...
  swipeDown(locator, yOffset, speed) {
    if (this.options.desiredCapabilities.platformName) {
      return this.browser.swipeDown(withStrictLocator(locator, this.options.desiredCapabilities.browserName), yOffset, speed);
    } else
  ...
  swipeLeft(locator, xoffset, speed) {
    if (this.options.desiredCapabilities.platformName) {
      return this.browser.swipeLeft(withStrictLocator(locator, this.options.desiredCapabilities.browserName), xoffset, speed);
    } else
  ...
  swipeRight(locator, xoffset, speed) {
    if (this.options.desiredCapabilities.platformName) {
      return this.browser.swipeRight(withStrictLocator(locator, this.options.desiredCapabilities.browserName), xoffset, speed);
    } else
  ...
  swipeUp(locator, yOffset, speed) {
    if (this.options.desiredCapabilities.platformName) {
      return this.browser.swipeUp(withStrictLocator(locator, this.options.desiredCapabilities.browserName), yOffset, speed);
    } else
  ...
  /**
   * Perform a swipe up on an element to element.
   * Appium: support Android
   */
  swipeTo(seachableLocator, scrollLocator, direction, timeout, offset, speed) {
    ...
    return browser.waitUntil(function() {
      if (err) {
        ...
        return browser.isVisible(withStrictLocator(seachableLocator, browserCap))
          .then(function(res) {
            if (res) {
              return true;
            } else {
   ...
   * Appium: support ??
   Android (not wotking)
   https://github.com/webdriverio/webdriverio/issues/1412
   */
  hold(locator) {
    if (this.options.desiredCapabilities.platformName) {
