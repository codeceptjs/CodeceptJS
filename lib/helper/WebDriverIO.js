...
class WebDriverIO extends Helper {
  ...
  /**
   * Retrieves a cssProperty from an element located by CSS or XPath and returns it to test.
   * Resumes test execution, so **should be used inside a generator with `yield`** operator.
   *
   * ```js
   * let pin = yield I.grabCssFrom('#pin', 'width');
   * ```
   */
  grabCssFrom(locator, cssProperty) {
    return this.browser.getCssProperty(withStrictLocator(locator), cssProperty).then(function (prop) {
      return prop;
    });
  }
