...
class Protractor extends Helper {
  ...
  async selectOption(select, option) {
    ...
    for (let key in option) {
      let opt = xpathLocator.literal(option[key]);
...
async function proceedSeeInField(assertType, field, value) {
  ...
  if (tag == 'select') {
    ...
    let textEl = await el.findElement(global.by.xpath(Locator.select.byValue(literal)));
    assertElementExists(textEl, `Option with value '${literal}'`, '');
