...
class Protractor extends SeleniumWebdriver {
  ...
  _initProtractor() {
    let ptor = requireg('protractor');
    global.by = new ptor.ProtractorBy;
    EC = new ptor.ProtractorExpectedConditions;
    if (!EC.presenceOf || !global.by.css) {
      // try to init Protractor 4;
      this._initProtractor4();
    }
  }
  ...
  _startBrowser() {
    ...
    if (protractorWrapper) {
      this.browser = protractorWrapper(this.options);
    } else {
