'use strict';
let webdriver, by, protractorWrapper, protractorPlugins, EC;
...
class Protractor extends Helper {
  ...
  constructor(config) {
    ...
    protractorPlugins = require('protractor/lib/plugins');
    ...
    if (config.proxy) this.options.capabilities.proxy = config.proxy;
    if (config.browser) this.options.capabilities.browserName = config.browser;

    global.by = by;

    // this.plugins = new Plugins(this.options);
    this.driverProvider = require('protractor/lib/driverProviders/'+this.options.driver)(this.options);
  }

  _init() {
    return this.driverProvider.setupEnv();
  }
