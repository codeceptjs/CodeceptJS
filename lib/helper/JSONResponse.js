...
class JSONResponse extends Helper {
  ...
  seeResponseCodeIs(code) {
    ...
    expect(this.response.status).toEqual(code, 'Response code is not the same as expected');
  }
  ...
  dontSeeResponseCodeIs(code) {
    this._checkResponseReady();
    expect(this.response.status).not.toEqual(code);
  }
  ...
  seeResponseContainsKeys(keys = []) {
    this._checkResponseReady();
    if (Array.isArray(this.response.data)) {
      this.response.data.forEach(data => expect(data).toHaveProperty(keys));
    } else {
      expect(this.response.data).toHaveProperty(keys);
    }
  }
   ...
   *   expect(data).keys.toContain(['user', 'company']);
   * });
   * ```
   *
   * @param {function} fn
  ...
  seeResponseEquals(resp) {
    this._checkResponseReady();
    expect(this.response.data).toEqual(resp);
  }
