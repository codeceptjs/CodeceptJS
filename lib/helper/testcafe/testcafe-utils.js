...
const getFixtureName = (param) => {

  if ([undefined, null].includes(param)) {
    return 'fixture';
  }

  if (Object.keys(param).includes('title')) {
    return param.title;
  }

  if (Object.keys(param).includes('file')) {
    return param.file;
  }
...
const createTestFile = (param) => {
  ...
  const fixtureName = getFixtureName(param);
