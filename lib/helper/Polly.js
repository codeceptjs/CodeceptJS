...
const pollyWebDriver = require('./PollyWebDriverExt');
...
class Polly extends Helper {
  ...
  _mockRequestForWebDriver(method, oneOrMoreUrls, dataOrStatusCode, additionalData) {
  ...
  _checkIfMockingStarted() {
    return (this.polly && this.polly.server) || this.browser;
  }
  ...
  async stopMocking() {
    ...
    if (this.currentDriver === 'Puppeteer') {
      ...
      this.browser.execute(pollyWebDriver.stopMocking);
      delete this.browser;
    }
  }
