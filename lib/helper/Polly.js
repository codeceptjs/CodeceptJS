...
const pollyWebDriver = require('./PollyWd');
...
class Polly extends Helper {
  ...
  async _startMocking(title = 'Test') {
    if (!(this.helpers && (this.helpers.Puppeteer || this.helpers.WebDriverIO))) {
      throw new Error('Puppeteer and WebDriverIO are the only supported helpers right now');
    }
    if (this.helpers.Puppeteer) await this._connectPuppeteer(title);
    else await this._connectWebDriver(title);
  }
  ...
  // Connect WebDriver helper to mock future requests.
  async _connectWebDriver(title) {
    const { browser } = this.helpers.WebDriverIO;
    browser.execute(pollyWebDriver.setup, title);
  }
