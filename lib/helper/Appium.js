...
class Appium extends WebdriverIO {
  ...
  switchToWeb(context) {
    ...
    return this.grabAllContexts().then((contexts) => {
      for (context in contexts) {
        if (contexts.match(/^WEBVIEW/)) return this._switchToContext(context);
      }
  ...
  selectOption(select, option) {
    if (!this.isWeb) throw new Error(`Should be used only in Web context. In native context use 'click' method instead`);
    return super.selectOption(parseLocator.call(this, select), option);
  }
...
function onlyForApps(expectedPlatform) {
  ...
  if (!expectedPlatform) {
    ...
    if (this.platform != expectedPlatform) {
