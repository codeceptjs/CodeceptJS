...
class Appium extends Webdriver {
  ...
  _validateConfig(config) {
    ...
    config.capabilities.deviceName =
      config.device || config.capabilities.deviceName;
    config.capabilities.browserName =
      config.browser || config.capabilities.browserName;
    config.capabilities.app = config.app || config.capabilities.app;
    config.capabilities.platformName =
      config.platform || config.capabilities.platformName;
  ...
  async switchToWeb(context) {
    ...
    for (const idx in contexts) {
      if (contexts[idx].match(/^WEBVIEW/))
        return this._switchToContext(contexts[idx]);
    }
...
function parseLocator(locator) {
  ...
  if (locator.type === 'id' && !this.isWeb && this.platform === 'android')
    return `//*[@resource-id='${locator.value}']`;
