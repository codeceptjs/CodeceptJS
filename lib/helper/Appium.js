...
class Appium extends Webdriver {
  ...
  /**
   * Send UNICODE_CHARACTER Key to route session/:session_id/keys with body: { value: [UNICODE_CHARACTER]}
   *
   * ```js
   * I.sendWinAppDriverKeys('UNICODE_CHARACTERS');
   * ```
   * @param unicode_char String UNICODE_CHARACTERS Sample: 'Tab': '\uE004'.
   *
   * Appium: support only Windows
   */
  async sendWinAppDriverKeys(unicode_char) {
    onlyForApps.call(this, 'Windows');
    
    return axios({
      method: 'post',
      url: `${this._buildAppiumEndpoint()}/session/${this.browser.sessionId}/keys`,
      data: { value: unicode_char },
    });
  }    
}
