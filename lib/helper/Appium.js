...
class Appium extends WebdriverIO {
  ...
  checkAppIsInstalled(bundleId) {
    onlyForApps.call(this, arguments.calle.name, "Android")
  ...
  seeOrientationIs(orientation) {
    onlyForApps.call(this, arguments.calle.name)
   ...
   */
  closeApp() {
    onlyForApps.call(this, arguments.calle.name, "iOS")
...
function onlyForApps(name, platformName) {
  if (!platformName) {
    if (!this.options.desiredCapabilities.platformName == platformName) {
      throw new Error(`${name} method can be used only with ${platformName} apps`)
    }
  } else {
    if (!this.options.desiredCapabilities.platformName) {
      throw new Error(`${name} method can be used only with apps`)
    }
  }
}
