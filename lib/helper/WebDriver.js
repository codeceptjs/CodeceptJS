...
class WebDriver extends Helper {
  ...
  */
  ...
  async seeTitleEquals(text) {
    const title = await this.browser.getTitle();
    return assert.toEqual(title, text, `expected web page title to be ${text}, but found ${title}`);
  }
  ...
  async seeNumberOfElements(locator, num) {
    ...
    return assert.toEqual(res.length, num, `expected number of elements (${(new Locator(locator))}) is ${num}, but found ${res.length}`);
  }
  ...
  async seeNumberOfVisibleElements(locator, num) {
    ...
    return assert.toEqual(res, num, `expected number of visible elements (${(new Locator(locator))}) is ${num}, but found ${res}`);
  }
