...
class WebDriver extends Helper {
  ...
  async grabElementBoundingRect(locator, prop) {
    ...
    const arr1 = await forEachAsync(res, async el => el.getLocation());
    const arr2 = await forEachAsync(res, async el => el.getSize());
    const arr = [];
    for (let index = 0; index < arr1.length; index++) {
      arr.push(Object.assign({}, arr1[index], arr2[index]));
    }
    const result = [];
    if (prop) {
      arr.forEach((element) => {
