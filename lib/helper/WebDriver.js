...
class WebDriver extends Helper {
  constructor(config) {
    ...
    let packageJsonFile = path.join(require.resolve('webdriverio'), '/../../', 'package.json');

    if (!fileExists(packageJsonFile)) {
      packageJsonFile = path.join(require.resolve('webdriverio'), '/../../../', 'package.json');
    }

    try {
      version = JSON.parse((fs.readFileSync(packageJsonFile)).toString()).version;
    } catch (err) {
