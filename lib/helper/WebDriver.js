...
class WebDriver extends Helper {
  ...
  async _startBrowser() {
    ...
    $$ = this.browser.$$.bind(this.browser);
    return this.browser;
  }
  ...
  async _after() {
    ...
    if (this.browser.isInsideFrame) await this.browser.switchToFr(null);
  ...
  _session() {
    const defaultSession = this.browser.sessionId;
    return {
      start: async (opts) => {
      ...
      loadVars: async (sessionId) => {
        ...
        this.browser.sessionId = sessionId;
      },
      restoreVars: async () => {
        if (this.context !== this.root) await this._withinEnd();
        this.browser.sessionId = defaultSession;
      },
    };
  }
  ...
  async _withinBegin(locator) {
    ...
    if (frame) {
      ...
      if (Array.isArray(frame)) {
        this.browser.switchTo(null);
        forEachAsync(frame, async f => this.switchTo(f));
        return;
      }
  ...
  async _locate(locator, smartWait = false) {
    if (!this.options.smartWait || !smartWait) {
      const els = await $$(withStrictLocator(locator));
      this.debugSection('Elements', `Found ${els.length} elements`);
      return els;
    }
  ...
  async _locateCheckable(locator) {
    return findCheckable.call(this, locator, $$.bind(this)).then(res => res);
  }
  ...
  async _locateClickable(locator) {
    return findClickable.call(this, locator, $$.bind(this)).then(res => res);
  }
  ...
  async grabTextFrom(locator) {
    const res = await this._locate(locator, true);
    assertElementExists(res, locator);
    const val = await forEachAsync(res, async el => this.browser.getElementText(el.ELEMENT));
  ...
  async seeElementInDOM(locator) {
    const res = await $$(withStrictLocator(locator));
  ...
  async grabCookie(name) {
    const cookie = await this.browser.getCookies([name]);
    this.debugSection('Cookie', cookie);
    return cookie[0];
  }
  ...
  async waitForText(text, sec = null, context = null) {
    ...
    return this.browser.waitUntil(
      async () => {
        const res = await $$(withStrictLocator.call(this, _context));
