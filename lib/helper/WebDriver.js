...
class WebDriver extends Helper {
  constructor(config) {
    ...
    // BiDi protocol enhancements
    this.bidiEnabled = false
    this.bidiNetworkEvents = []
    this.bidiScriptExceptions = []
    this.bidiConsoleMessages = []
    this.bidiPerformanceMetrics = []
    this.bidiNavigationEvents = []
  ...
  _validateConfig(config) {
    ...
    // Set BiDi enabled flag for enhanced features
    this.bidiEnabled = config.bidiProtocol || false
  ...
  async _startBrowser() {
    ...
    // Enhanced BiDi Protocol Support
    if (this.bidiEnabled) {
      await this._initializeBiDiProtocol()
    } else {
      // Basic log subscription for backward compatibility
  ...
  /**
   * Initialize comprehensive BiDi Protocol support
   * Sets up event subscriptions for network, console, script exceptions, and performance monitoring
   * @private
   */
  async _initializeBiDiProtocol() {
    if (!this.bidiEnabled) return

    this.debugSection('BiDi', 'Initializing comprehensive BiDi Protocol support')

    try {
      // Subscribe to comprehensive BiDi events
      const bidiEvents = [
        'log.entryAdded',
      ...
      await this.browser.sessionSubscribe({ events: bidiEvents })
      this.debugSection('BiDi', `Subscribed to ${bidiEvents.length} BiDi events`)

      // Set up enhanced event handlers
      this._setupBiDiEventHandlers()
    } catch (error) {
      this.debugSection('BiDi', `Warning: Could not initialize all BiDi features: ${error.message}`)
      // Fallback to basic log subscription
      await this.browser.sessionSubscribe({ events: ['log.entryAdded'] })
      this.browser.on('log.entryAdded', logEvents)
    }
  }
  ...
  /**
   * Set up BiDi event handlers for comprehensive monitoring
   * @private
   */
  _setupBiDiEventHandlers() {
    // Enhanced log event handling
    this.browser.on('log.entryAdded', event => {
      browserLogs.push(event.text)
      this.bidiConsoleMessages.push({
    ...
    this.browser.on('network.beforeRequestSent', event => {
      this.bidiNetworkEvents.push({
        type: 'request',
        timestamp: new Date().toISOString(),
    ...
    this.browser.on('network.responseStarted', event => {
      this.bidiNetworkEvents.push({
    ...
    this.browser.on('network.responseCompleted', event => {
      this.bidiNetworkEvents.push({
        ...
        url: event.response.url,
        status: event.response.status,
        responseTime: event.response.timings?.total,
      })
    })

    // Network error monitoring
    this.browser.on('network.fetchError', event => {
      this.bidiNetworkEvents.push({
        type: 'fetchError',
        timestamp: new Date().toISOString(),
        requestId: event.request.requestId,
    ...
    this.browser.on('browsingContext.navigationStarted', event => {
      this.bidiNavigationEvents.push({
    ...
    this.browser.on('browsingContext.domContentLoaded', event => {
      this.bidiNavigationEvents.push({
    ...
    this.browser.on('browsingContext.load', event => {
      this.bidiNavigationEvents.push({
        type: 'load',
        timestamp: new Date().toISOString(),
        context: event.context,
    ...
    // Script realm monitoring for performance and errors
    this.browser.on('script.realmCreated', event => {
      this.debugSection('BiDi', `Script realm created: ${event.realm}`)
    })

    this.browser.on('script.realmDestroyed', event => {
      this.debugSection('BiDi', `Script realm destroyed: ${event.realm}`)
    })
  }

  /**
   * Get comprehensive network events captured via BiDi protocol
   *
   * ```js
   * const networkEvents = await I.grabBiDiNetworkEvents();
   * console.log('Network requests:', networkEvents.filter(e => e.type === 'request'));
   * ```
   *
   * @returns {Array} Array of network events with detailed information
   */
  async grabBiDiNetworkEvents() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }
    this.debugSection('BiDi Network', `Retrieved ${this.bidiNetworkEvents.length} network events`)
    return [...this.bidiNetworkEvents]
  }

  /**
   * Get enhanced console messages captured via BiDi protocol
   *
   * ```js
   ...
   * ```
   *
   * @returns {Array} Array of console messages with enhanced metadata
   */
  async grabBiDiConsoleMessages() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }
    this.debugSection('BiDi Console', `Retrieved ${this.bidiConsoleMessages.length} console messages`)
    return [...this.bidiConsoleMessages]
  }

  /**
   * Get navigation events captured via BiDi protocol
   *
   * ```js
   * const navigationEvents = await I.grabBiDiNavigationEvents();
   * const pageLoads = navigationEvents.filter(e => e.type === 'load');
   * ```
   *
   * @returns {Array} Array of navigation events
   */
  async grabBiDiNavigationEvents() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }
    this.debugSection('BiDi Navigation', `Retrieved ${this.bidiNavigationEvents.length} navigation events`)
    return [...this.bidiNavigationEvents]
  }

  /**
   * Clear all BiDi collected events
   *
   * ```js
   * I.clearBiDiEvents();
   * ```
   */
  async clearBiDiEvents() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }
    this.bidiNetworkEvents = []
    ...
    this.bidiScriptExceptions = []
    this.bidiPerformanceMetrics = []
    this.debugSection('BiDi', 'Cleared all BiDi events')
  }

  /**
   * Wait for a specific network request using BiDi protocol
   *
   * ```js
   * await I.waitForBiDiNetworkEvent({
   *   url: 'https://api.example.com/users',
   *   method: 'GET'
   * });
   * ```
   *
   * @param {Object} options - Network event criteria
   * @param {string} [options.url] - URL to match (supports partial matching)
   * @param {string} [options.method] - HTTP method to match
   * @param {number} [options.timeout=5000] - Timeout in milliseconds
   * @returns {Promise<Object>} The matched network event
   */
  async waitForBiDiNetworkEvent(options = {}, timeout = 5000) {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }
    ...
    return new Promise((resolve, reject) => {
      const checkInterval = setInterval(() => {
        const matchedEvent = this.bidiNetworkEvents.find(event => {
          if (event.type !== 'request') return false
          if (url && !event.url.includes(url)) return false
        ...
        if (matchedEvent) {
          ...
          this.debugSection('BiDi Network', `Found matching network event: ${matchedEvent.url}`)
          resolve(matchedEvent)
        } else if (Date.now() - startTime > timeout) {
          clearInterval(checkInterval)
          reject(new Error(`Network event not found within ${timeout}ms. Criteria: ${JSON.stringify(options)}`))
        }
      }, 100)
    })
  }
  ...
  /**
   * Execute script with BiDi protocol enhanced error handling
   *
   * ```js
   ...
   */
  async executeBiDiScript(script, ...args) {
    if (!this.bidiEnabled) {
      return this.executeScript(script, ...args)
    }

    try {
      this.debugSection('BiDi Script', 'Executing script with enhanced error handling')
      const result = await this.browser.execute(script, ...args)
      return result
    } catch (error) {
      this.bidiScriptExceptions.push({
        timestamp: new Date().toISOString(),
  ...
  /**
   * Monitor performance metrics using BiDi protocol
   *
   * ```js
   ...
   * ```
   */
  async startBiDiPerformanceMonitoring() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }

    this.debugSection('BiDi Performance', 'Starting performance monitoring')
    this.bidiPerformanceMetrics = []

    // Inject performance observer
    await this.executeScript(() => {
      if (window.bidiPerformanceObserver) return

      window.bidiPerformanceMetrics = []

      const observer = new PerformanceObserver(list => {
        for (const entry of list.getEntries()) {
          window.bidiPerformanceMetrics.push({
            name: entry.name,
      ...
      window.bidiPerformanceObserver = observer
    })
  }

  /**
   * Get collected performance metrics via BiDi protocol
   *
   * @returns {Promise<Array>} Array of performance metrics
   */
  async getBiDiPerformanceMetrics() {
    if (!this.bidiEnabled) {
      throw new Error('BiDi protocol is not enabled. Set bidiProtocol: true in WebDriver configuration.')
    }

    const metrics = await this.executeScript(() => {
      return window.bidiPerformanceMetrics || []
    })

    this.bidiPerformanceMetrics = metrics
    this.debugSection('BiDi Performance', `Retrieved ${metrics.length} performance metrics`)
    return metrics
  }
}
