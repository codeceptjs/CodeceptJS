...
class Nightmare extends Helper {
  ...
  async seeInTitle(text) {
    let title = await this.browser.title();
  ...
  async seeInCurrentUrl(url) {
    let currentUrl = await this.browser.url();
  ...
  async seeElement(locator) {
    locator = new Locator(locator, 'css');
    let num = await this.browser.evaluate((by, locator) => {
  ...
  async seeElementInDOM(locator) {
    locator = new Locator(locator, 'css');
    let els = await this.browser.findElements(locator.toStrict());
  ...
  async seeInSource(text) {
    let source = await this.browser.evaluate(() => document.documentElement.outerHTML);
  ...
  async click(locator, context = null) {
    let el = await findClickable.call(this, locator, context);
  ...
  async moveCursorTo(locator, offsetX = 0, offsetY = 0) {
    locator = new Locator(locator, 'css');
    let el = await this.browser.findElement(locator.toStrict());
  ...
  async checkOption(field, context = null) {
    let els = await findCheckable.call(this, field, context);
  ...
  async fillField(field, value) {
    let el = await findField.call(this, field);
  ...
  async grabValueFrom(locator) {
    let el = await findField.call(this, locator);
  ...
  async selectOption(select, option) {
    ...
    let el = await findField.call(this, select);
  ...
  async seeCookie(name) {
    let res = await this.browser.cookies.get(name);
...
async function proceedSee(assertType, text, context) {
  ...
  let texts = await this.browser.evaluate((by, locator) => {
