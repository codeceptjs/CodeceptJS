...
class Nightmare extends Helper {
    ...
    async _startBrowser() {
        ...
        if (this.options.enableHAR) {
            this.browser = this.Nightmare(
                Object.assign(require('nightmare-har-plugin').getDevtoolsOptions(), this.options),
            )
            await this.browser
        ...
        for (let index = 0; index < els.length; index++) {
            ...
            array.push(
                await this.browser.evaluate(
                    (el, attr) => window.codeceptjs.fetchElement(el).getAttribute(attr),
                    el,
                    attr,
                ),
            )
        ...
        return this.browser
            ...
            .catch((err) => {
                if (err.message.indexOf('Cannot read property') > -1) {
                    ...
                    throw new Error(
                        `there is no element(${JSON.stringify(context)}) with text "${text}" after ${sec} sec`,
                    )
                } else throw err
            })
    }
