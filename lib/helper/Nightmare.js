...
class Nightmare extends Helper {
  ...
  _withinBegin(locator , frames) {
    this.context = locator;
    locator = guessLocator(locator) || {css: locator};
    this.browser.evaluate(function (by, locator, frames) {
      var el;
      if(!frames){
        el = codeceptjs.findElement(by, locator);
      }
      else{
        el = frames.reduce((parent,child)=>parent.querySelector(child).contentDocument , window.document );
      }
      if (!el) throw new Error(`Element by ${by}: ${locator} not found`);
      window.codeceptjs.within = el;
    }, lctype(locator), lcval(locator) , frames );
  }
