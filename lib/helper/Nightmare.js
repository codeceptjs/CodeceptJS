...
import { includes: stringIncludes } from '../assert/include.js'
import { urlEquals } from '../assert/equal.js'
import { equals } from '../assert/equal.js'
import { empty } from '../assert/empty.js'
import { truth } from '../assert/truth.js'
import Locator from '../locator.js'
import ElementNotFound from './errors/ElementNotFound.js'
import { xpathLocator, fileExists, screenshotOutputFolder, toCamelCase } from '../utils.js'
...
class Nightmare extends Helper {
  ...
  static _checkRequirements() {
    try {
      (await import('nightmare')).default
    } catch (e) {
      return ['nightmare']
    }
  }
  ...
  async _init() {
    this.Nightmare = (await import('nightmare')).default

    if (this.options.enableHAR) {
      (await import('nightmare-har-plugin')).default.install(this.Nightmare)
    }
  ...
  async _startBrowser() {
    ...
    if (this.options.enableHAR) {
      this.browser = this.Nightmare(Object.assign((await import('nightmare-har-plugin')).default.getDevtoolsOptions(), this.options))
      await this.browser
  ...
  async saveHAR(fileName) {
    ...
    await this.browser.getHAR().then(har => {
      (await import('fs')).default.writeFileSync(outputFile, JSON.stringify({ log: har }))
    })
  }
