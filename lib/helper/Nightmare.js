...
class Nightmare extends Helper {
   ...
   *
   * ```js
   * I.amOnPage('/ajax', [{'x-my-custom-header': 'some value'}])
   * ```
   */
  ...
  click(locator, context) {
    ...
    return co(findClickable.call(this, locator, context)).then((el) => {
      ...
      }, el).wait(this.options.clickWait); // wait for click event to happen
    });
  }
  ...
  pressKey(key) {
    ...
    return this.browser.pressKey(key).wait(this.options.clickWait);
  }
