...
class REST extends Helper {
  ...
  seeHttpHeader(name, value = undefined) {
    expect(this.response.headers).to.have.property(name);
    if (value) {
      expect(this.response.headers[name]).to.equal(value);
    }
  }
  ...
  dontSeeHttpHeader(name, value = undefined) {
    expect(this.response.headers).to.not.have.property(name);
    if (value) {
      expect(this.response.headers[name]).to.equal(value);
    }
  }
