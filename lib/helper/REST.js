 ...
 */
  seeResponseCodeIs(statusCode) {
    assert.ok(this.response.status === statusCode, `Status code ${this.response.status} of response is not equal to ${statusCode}`);
  }
  ...
  dontSeeResponseCodeIs(statusCode) {
    assert.ok(this.response.status !== statusCode, `Status code ${this.response.status} of response is equal to ${statusCode}`);
  }
  ...
  seeResponseWasSucessful() {
    const max = 300;
    const min = 199;
    const assertion = min < this.response.status && this.response.status < max;
    assert.ok(assertion, `status code is between ${min} and ${max}, however it was ${this.response.status}`);
  }
  ...
  seeHeader(key, value = undefined) {
    const foundKey = Object.keys(this.response.headers).some(header => header === key);
    assert.ok(foundKey, `${key} was not found in headers`);
    if (value) {
      assert.ok(this.response.headers[key] === value, `expected ${key} to be ${value}, however it was ${this.response.headers[key]}`);
    }
  }
   ...
   */
  dontSeeHeader(key) {
    const keyNotFound = Object.keys(this.response.headers).every(header => header !== key);
    assert.ok(keyNotFound, `expected ${key} to be not found in headers, however it was found`);
  }
   ...
   */
  seeResponseBody(obj) {
    const limitedEquals = JSON.stringify(obj) === JSON.stringify(this.response.data);
    assert.ok(limitedEquals, 'objects are not equal');
  }
}
module.exports = REST;
