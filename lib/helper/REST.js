...
class REST extends Helper {
  ...
  async _executeRequest(request) {
    ...
    if (request.data) {
      ...
      if (typeof request.data === 'object' && !fetchOptions.headers['Content-Type']) {
    ...
    // Support for httpsAgent is limited in native fetch
    // If needed, we might need to use a library like undici for more advanced cases
    if (this.httpsAgent) {
      this.debug('Warning: httpAgent configuration is not fully supported with native fetch.')
    }
