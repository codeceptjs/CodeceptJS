...
async function findByPlaywrightLocator(matcher, locator) {
  if (locator && locator.toString().includes(process.env.testIdAttribute)) return matcher.getByTestId(locator.playwright.value.split('=')[1]);
  return matcher.locator(locator.playwright).all();
}
