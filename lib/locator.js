...
class Locator {
  ...
  with(locator) {
    const xpath = sprintf('%s%s', this.toXPath(), (new Locator(locator, 'css')).toXPath());
    return new Locator({ xpath });
  }
  ...
  at(position) {
    ...
    const xpath = sprintf('(%s)[position()=%s]',
      this.toXPath(), position);
    return new Locator({ xpath });
  }
  ...
  inside(locator) {
    const xpath = sprintf('%s/ancestor::%s', this.toXPath(), (new Locator(locator, 'css')).toXPath().slice(3));
    return new Locator({ xpath });
  }

  near(locator) {
    let xpath = sprintf('%s/following-sibling::%s', this.toXPath(), (new Locator(locator, 'css')).toXPath().slice(3));
    xpath += sprintf(' | %s/preceding-sibling::%s', this.toXPath(), (new Locator(locator, 'css')).toXPath().slice(3));
    return new Locator({ xpath });
  }
}
