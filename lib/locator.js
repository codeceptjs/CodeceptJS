...
class Locator {
  ...
  getTypeOrDefault(defaultType) {
    if (this.isFuzzy() || this.isNull()) return defaultType;
    return this.type;
  }
...
Locator.select = {
  byVisibleText: (opt) => {
    let normalized = `[normalize-space(.) = "${opt.trim() }"]`;
    return `./option${normalized}|./optgroup/option${normalized}`;
  },

  byValue: (opt) => {
    let normalized = `[normalize-space(@value) = "${opt.trim() }"]`;
    return `./option${normalized}|./optgroup/option${normalized}`;
  },
