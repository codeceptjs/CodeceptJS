...
module.exports.serializeStepArgs = function (stepArgs) {
  const _res = stepArgs.map(item => {
    if (item) {
      if (item && item._secret) return '*****';
      if (item.type === 'object' && item.$_root) return 'Joi schema';
