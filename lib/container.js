...
async function loadSupportObject(modulePath, supportObjectName) {
  ...
  // Try to import the module, adding .js extension if needed for ESM
  let module
  try {
    module = await import(modulePath);
  } catch (err) {
    // If the import fails and we're dealing with a relative path without extension,
    // try adding .js extension for ESM compatibility
    if (err.code === 'ERR_MODULE_NOT_FOUND' && !modulePath.endsWith('.js') && !modulePath.endsWith('.mjs') && !modulePath.endsWith('.ts')) {
      try {
  ...
  try {
    ...
    throw new Error(`Could not process support object ${supportObjectName}: ${err.message}`)
  }
}
