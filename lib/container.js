...
const { methodsOfObject, fileExists, isFunction, isAsyncFunction, installedLocally } = utilsModule.default || utilsModule
          ...
          asyncHelperPromise = asyncHelperPromise
            ...
            .then(ResolvedHelperClass => {
              if (typeof ResolvedHelperClass !== 'function') {
                throw new Error(`Helper '${helperName}' is not a class.`)
              }
    ...
    try {
      ...
      if (err.code === 'ERR_REQUIRE_ESM' || (err.message && err.message.includes('ES module'))) {
        ...
        try {
          ...
          const absolutePath = pathModule.resolve(moduleName)
