...
import mochaFactoryModule from './mocha/factory.js'
const MochaFactory = mochaFactoryModule.default || mochaFactoryModule
      ...
      {},
      {
        get(target, prop) {
          ...
          if (!container.support[name]) {
            ...
            const supportObject = loadSupportObjectSync(config[name])
...
function createActor(actorPath) {
  ...
  if (actorPath) {
    container.support.I = loadSupportObjectSync(actorPath)
  } else {
...
function loadPluginSync(modulePath, config) {
  let pluginMod
  try {
    pluginMod = require(modulePath)
  } catch (err) {
    if (err.code === 'ERR_REQUIRE_ESM' || (err.message && err.message.includes('ES module'))) {
      throw new Error(`ESM plugin '${modulePath}' cannot be loaded synchronously. Please use CommonJS format or configure for ESM support.`)
    }
    throw err
  }
...
async function createPlugins(config, options = {}) {
  ...
  for (const pluginName in config) {
    ...
    try {
      ...
      // Try async loading first for ESM modules
      try {
        plugins[pluginName] = await loadPluginAsync(module, config[pluginName])
        debug(`plugin ${pluginName} loaded via async import`)
      } catch (asyncErr) {
        // Fall back to sync loading for CommonJS modules
        try {
          plugins[pluginName] = loadPluginSync(module, config[pluginName])
          debug(`plugin ${pluginName} loaded via sync require`)
        } catch (syncErr) {
          throw new Error(`Could not load plugin ${pluginName} from module '${module}':\nAsync error: ${asyncErr.message}\nSync error: ${syncErr.message}`)
        }
      }
    } catch (err) {
      throw new Error(`Could not load plugin ${pluginName} from module '${module}':\n${err.message}\n${err.stack}`)
...
function loadSupportObjectSync(modulePath, supportObjectName) {
  ...
  try {
    ...
    try {
      ...
      if (err.code === 'ERR_REQUIRE_ESM' || (err.message && err.message.includes('ES module'))) {
        throw new Error(`ESM module '${modulePath}' cannot be loaded synchronously. Please use CommonJS format or configure for ESM support.`)
      }
      throw err
    }
