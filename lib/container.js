...
global.codecept_dir = global.codecept_dir || getTestRoot();
global.Helper = global.Helper || require('../lib/helper');
...
function createHelpers(config) {
  ...
  for (const helperName in config) {
    try {
      ...
      let helpers = { helperName };
      
      if (typeof HelperClass === 'object') {
        Object.values(HelperClass).forEach(helper => {
          helpers[helperName] = new helper(config[helperName])
        })
      } else {
