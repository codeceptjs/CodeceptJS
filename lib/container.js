...
class Container {
  ...
  static addWorker(workerObj) {
    container.workerObjects[workerObj.threadId] = workerObj;
  ...
  static share(data) {
    ...
    if (isMainThread) {
      for (const workerObj of Object.values(container.workerObjects)) {
...
const invokeWorkerListeners = (workerObj) => {
  ...
  workerObj.on('exit', () => {
    delete container.workerObjects[threadId];
  });
};
