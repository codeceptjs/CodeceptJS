    ...
    Object.keys(currentObject).forEach((method) => {
      ...
      if (typeof currentMethod === 'function') {
        if (currentMethod[Symbol.toStringTag] === 'AsyncFunction') {
          objects[object][method] = asyncWrapper(object, method, currentMethod);
        } else {
          objects[object][method] = functionWrapper(object, method, currentMethod);
        }
      }
    });
  });
...
function loadGherkinSteps(paths) {
  const { Given, When, Then } = require('./interfaces/bdd');
