...
export function log(msg) {
  if (outputLevel >= 3) {
    print(' '.repeat(stepShift), styles.log(truncate(`   ${msg}`, spaceShift)));
  }
}
...
export function debug(msg) {
  if (outputLevel >= 2) {
    print(' '.repeat(stepShift), styles.debug(`${figures.pointerSmall} ${msg}`));
  }
}
 ...
 * @param {string} [color]
 */
export function say(message, color = 'cyan') {
...
export function debugSection(section, msg) {
  debug(`[${section}] ${msg}`);
}
...
export function step(step) {
  ...
  if (step.metaStep && outputLevel >= 1) {
    // stepShift += 2;
    stepLine = colors.green(truncate(stepLine, spaceShift));
  }
  if (step.comment) {
    stepLine += colors.grey(step.comment.split('\n').join('\n' + ' '.repeat(4))); // eslint-disable-line
  }

  print(' '.repeat(stepShift), truncate(stepLine, spaceShift));
}
 ...
 * @param {string} msg
 */
export function plugin(pluginName, msg = '') {
  debug(`<${pluginName}> ${msg}`);
}
 ...
 */
export function result(passed, failed, skipped, duration) {
