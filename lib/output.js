...
module.exports = {
  ...
  step(step) {
    ...
    if (codeceptjs.config.get().plugins.customLocator && codeceptjs.config.get().plugins.customLocator.enabled) {
      if (step.args.length > 0) {
        for (let index = 0; index < step.args.length; index++) {
          const arg = step.args[index];
          if (new Locator(arg)) {
            const locator = new Locator(arg);
            step.args[index] = locator.output;
          }
        }
      }
    }
