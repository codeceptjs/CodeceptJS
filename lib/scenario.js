...
export function test(test) {
  ...
  test.fn = async function (done) {
    ...
    if (isAsyncFunction(testFn)) {
      ...
      testFn.call(test, injectedArguments).then(async () => {
        await recorder.add('fire test.passed', async () => {
          event.emit(event.test.passed, test);
          event.emit(event.test.finished, test);
        });
        await recorder.add('finish test', () => done());
        recorder.catch();
      }).catch(catchError);
      return;
    }
    ...
    try {
      ...
      await recorder.add('fire test.passed', () => {
        event.emit(event.test.passed, test);
        event.emit(event.test.finished, test);
      });
      await recorder.add('finish test', () => done());
