...
module.exports.injected = function (fn, suite, hookName) {
  return function (done) {
    ...
    promiseRetry(
      ...
      .then(() => {
        recorder.add("fire hook.passed", () =>
          event.emit(event.hook.passed, suite)
        );
      ...
      .catch((e) => {
        ...
        recorder.catch((e) => {
          const err =
            recorder.getAsyncErr() === null ? e : recorder.getAsyncErr();
          errHandler(err);
        });
        recorder.add("fire hook.failed", () =>
          event.emit(event.hook.failed, suite, e)
        );
      });
  };
};
