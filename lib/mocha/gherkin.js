...
const gherkinDebug = debug('codeceptjs:bdd');
...
export default (text, file) => {
  ...
  const runSteps = async steps => {
    for (const step of steps) {
      ...
      try {
        gherkinDebug(`Step '${step.text}' started...`)
        await fn(...fn.params)
        gherkinDebug('Step passed')
        step.status = 'passed'
      } catch (err) {
        gherkinDebug(`Step failed: ${err?.message}`)
        step.status = 'failed'
        step.err = err
        throw err
      } finally {
...
function getTranslation(language) {
  const translations = Object.keys(allTranslations)
  ...
  for (const availableTranslation of translations) {
    ...
    if (availableTranslation.includes(language)) {
      return allTranslations[availableTranslation]
    }
  }
}
;