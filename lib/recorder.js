...
class Recorder {
  ...
  static session = RecorderSession;
   ...
   * @inner
   */
  static add(taskName, fn = undefined, force = false, retry = true) {
   ...
   * @inner
   */
  static catchWithoutStop(customErrFn) {
    return promise = promise.catch((err) => {
      log(`${currentQueue()}Error | ${err}`);
      if (!(err instanceof Error)) { // strange things may happen
        err = new Error(`[Wrapped Error] ${JSON.stringify(err)}`); // we should be prepared for them
  ...
  /** @param {*} err */
  static saveFirstAsyncError(err) {
    if (asyncErr === null) {
      asyncErr = err;
    }
  }
