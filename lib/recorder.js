...
const recorderDebug = debug('codeceptjs:recorder');
const MAX_TASKS = 100
...
const recorderModule = {
   ...
   */
  start() {
    recorderDebug('Starting recording promises')
    running = true
    asyncErr = null
    errFn = null
    this.reset()
  ...
  session: {
    ...
    start(name) {
      if (sessionId) {
        recorderDebug(`${currentQueue()}Session already started as ${sessionId}`)
        this.restore(sessionId)
      }
      recorderDebug(`${currentQueue()}Starting <${name}> session`)
      tasks.push('--->')
    ...
    restore(name) {
      tasks.push('<---')
      recorderDebug(`${currentQueue()}Finalize <${name}> session`)
      this.running = false
      sessionId = null
      this.catch(errFn)
  ...
  add(taskName, fn = undefined, force = false, retry = undefined, timeout = undefined) {
    ...
    tasks.push(taskName)
    recorderDebug(chalk.gray(`${currentQueue()} Queued | ${taskName}`))
    ...
    return (promise = Promise.resolve(promise).then(res => {
      ...
      recorderDebug(`${currentQueue()} Running | ${taskName} | Timeout: ${timeout || 'None'}`)
      if (retryOpts) recorderDebug(`${currentQueue()} Retry opts`, JSON.stringify(retryOpts))
   ...
   */
    ...
    const fnDescription = customErrFn
      ...
      .replace(/\n/g, ' ')
      ?.slice(0, 50)
    recorderDebug(chalk.gray(`${currentQueue()} Queued | catch with error handler ${fnDescription || ''}`))
...
function getTimeoutPromise(timeoutMs, taskName) {
  let timer
  if (timeoutMs) recorderDebug(`Timing out in ${timeoutMs}ms`)
  return [
