...
class Config {
  ...
  /**
   * Get object with absolute paths
   * @param {string} key
   * @return {*}
   */
  static absolutifyPaths(key) {
    if (!key || !config[key]) {
      return {};
    }

    const dataObject = { ...config[key] };

    Object.keys(dataObject).forEach(key => {
      if (typeof dataObject[key] === 'string' && dataObject[key] && !path.isAbsolute(dataObject[key])) {
        dataObject[key] = path.resolve(global.codecept_dir || '', dataObject[key]);
      }
    });
