...
class MetaStep extends Step {
  ...
  // @TODO: need tight type definition
  /** @return {void} */
  run(fn) {
    this.status = 'queued';
    ...
    event.emit(event.metaStep.before, this);

    const registerStep = (step) => {
      step.metaStep = this;
    };
    event.dispatcher.on(event.step.before, registerStep);
    event.emit(event.metaStep.started, this);
    this.startTime = Date.now();
    try {
      result = fn.apply(this.context, this.args);
      event.emit(event.metaStep.after, this);
      event.emit(event.metaStep.passed, this);
    } catch (error) {
      this.status = 'failed';
      event.emit(event.metaStep.started, this);
      event.emit(event.metaStep.failed, this);
    } finally {
      this.endTime = Date.now();
      event.emit(event.metaStep.finished, this);
    }
