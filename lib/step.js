...
// deprecated
let support;
// могут быть в каждом обекте свои
const setOfMetaStep = new Map();
/**
...
class MetaStep extends Step {
  constructor(obj, method) {
    super(null, method);
    this.actor = obj;
    this.activeMetaStep = undefined;
    this.methodArgs = [];
    setOfMetaStep.set(method, this);
  }
  ...
  /** @return {void} */
  run(fn, ...args) {
    console.log('from  metaStep: ');
    // console.log(fn);
    // console.log(args);
    this.args = args;
    this.activeMetaStep = this.helperMethod;
    console.log(this.activeMetaStep);
    return fn(args);
  }
}
...
function getMetaStepNameFromStack(stack) {
...
function detectMetaStep(stack) {
  ...
  for (let i = STACK_LINE; i < stack.length; i++) {
    ...
    if (fnName[1] === 'MetaStep') {
      // detect PO name from includes
      for (const name in support) {
        const file = support[name];
        if (stack[i - 1].indexOf(file) > -1) {
          const info = stack[i - 1].trim().match(/^at (\w+)\s\(/);
          return setOfMetaStep.get(info[1]);
        }
      }
    }
