...
module.exports = function (suite) {
  ...
  suite.on('pre-require', (context, file, mocha) => {
    ...
    context.xScenario = context.Scenario.skip = function (title, opts = {}, fn) {
      // Need for skip with msg
      if (context.Scenario.skipInfo) {
        if (typeof opts === 'function' && !fn) {
          fn = opts;
          opts = {};
        }
        context.Scenario.skipInfo = {
          ...context.Scenario.skipInfo,
          description: fn.toString(),
        };

        return context.Scenario(title, {
          skipInfo: context.Scenario.skipInfo,
        });
      }
    ...
    context.Scenario.todo = function (title, opts = {}, fn) {
      ...
      return context.Scenario(title, { skipInfo });
    };

    context.Skip = function (opts = {}) {
      if (!opts.message) throw new Error('message is required param for skip scenario.');
      context.Scenario.skipInfo = opts;
      return {
        Scenario: context.Scenario.skip,
      };
    };
