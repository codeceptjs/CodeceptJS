...
module.exports = function (suite) {
  ...
  suite.on('pre-require', function (context, file, mocha) {
    ...
    let addScenario = function (title, data, opts, fn, testData) {
      var suite = suites[0];
      ...
      if (data instanceof DataTable) {
        ...
        data.datatable.forEach(function (scenario) {
          addScenario(`${title} ${JSON.stringify(scenario)}`, opts, fn, scenario);
        });
        return;
      } else {
        testData = fn;
        ...
        opts = data;
        data = {};
        if (suite.codeceptData && !testData) {
          suite.codeceptData.datatable.forEach(function (scenario) {
            addScenario(`${title} ${JSON.stringify(scenario)}`, opts, fn, scenario);
          });
          return;
        }
    ...
    context.Feature = function (title, data, opts) {
      if (suites.length > 1) {
        suites.shift();
      }
