...
const isDocumentedMethod = doclet => doclet.undocumented !== true && doclet.kind === 'function' && doclet.scope === 'instance'
const shouldOverrideReturns = doclet => !doclet.returns || !doclet.returns[0].type || !doclet.returns[0].type.names[0].includes('Promise')
...
module.exports = {
  handlers: {
    ...
    newDoclet: ({ doclet }) => {
      if (isHelper(doclet.meta.path) && isDocumentedMethod(doclet) && shouldOverrideReturns(doclet)) {
        doclet.returns = []
